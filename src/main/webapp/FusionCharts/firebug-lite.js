(function() {
	...
	(function() {
		...
		this.initialize = function() {
			...
			FBL.Ajax.initialize();
			var isChromeContext = window.Firebug
					&& typeof window.Firebug.SharedEnv == "object";
			if (isChromeContext) {
				...
				if (FBL.isFirefox && typeof FBL.Env.browser.console == "object"
			...
			if (FBTrace.DBG_INITIALIZE && isChromeContext) {
				FBTrace.sysout("FBL.initialize - persistent application",
						"initialize chrome context")
			}
			if (FBTrace.DBG_INITIALIZE) {
				FBTrace.sysout("FBL.initialize", namespaces.length / 2
								+ " namespaces BEGIN")
			}
			...
			if (FBL.Env.isChromeExtension) {
				...
				if (!doc.getElementById("FirebugChannel")) {
					...
					doc.documentElement.insertBefore(channel,
		...
		var findLocation = function findLocation() {
			...
			FBL.Env.isChromeExtension = script
					&& script.getAttribute("extension") == "Chrome";
			if (FBL.Env.isChromeExtension) {
				path = productionDir;
				script = {
			...
			if (path && m) {
				...
				loc.skinDir = (isProductionRelease ? path : loc.baseDir)
						+ "skin/" + Env.skin + "/";
		...
		this.bind = function() {
			var args = cloneArray(arguments), fn = args.shift(), object = args
					.shift();
			return function() {
				return fn.apply(object, arrayInsert(cloneArray(args), 0,
								arguments))
			}
		};
		...
		this.createStyleSheet = function(doc, url) {
			var style = doc.createElementNS("http://www.w3.org/1999/xhtml",
					"link");
		...
		function addEntityMapToList(ccode, entity) {
			var lists = Array.prototype.slice.call(arguments, 2), len = lists.length, ch = String
					.fromCharCode(ccode);
			for (var i = 0; i < len; i++) {
				var list = lists[i];
		...
		function escapeGroupsForEntities(str, lists) {
			lists = [].concat(lists);
			var re = getEscapeRegexp("normal", lists), split = String(str)
					.split(re), len = split.length, results = [], cur, r, i, ri = 0, l, list, last = "";
			if (!len) {
				return [{
							str : String(str),
							group : "",
		...
		function unescapeEntities(str, lists) {
			var re = getEscapeRegexp("reverse", lists), split = String(str)
					.split(re), len = split.length, results = [], cur, r, i, ri = 0, l, list;
			if (!len) {
				return str
			}
			lists = [].concat(lists);
		...
		var escapeForElementAttribute = this.escapeForElementAttribute = createSimpleEscape(
				"attributes", "normal");
		var escapeForCss = this.escapeForCss = createSimpleEscape("css",
				"normal");
		...
		function escapeHTMLAttribute(value) {
			...
			return around + (String(value).replace(/[&'"]/g, replaceChars))
					+ around
		}
		...
		this.cropString = function(text, limit) {
			...
			if (text.length > limit) {
				return this.escapeNewLines(text.substr(0, halfLimit) + "..."
		...
		this.isVisible = function(elt) {
			return this.getStyle(elt, "visibility") != "hidden"
					&& (elt.offsetWidth > 0 || elt.offsetHeight > 0
							...
							|| elt.namespaceURI == "http://www.w3.org/2000/svg" || elt.namespaceURI == "http://www.w3.org/1998/Math/MathML")
		};
		...
		this.getClientOffset = function(elt) {
			var addOffset = function addOffset(elt, coords, view) {
				...
				var style = isIE ? elt.currentStyle : view.getComputedStyle(
						elt, "");
				if (elt.offsetLeft) {
		...
		this.applyBodyOffsets = function(elt, clientRect) {
			...
			if (pos === "absolute" || pos === "relative") {
				var borderLeft = parseInt(style
								.getPropertyValue("border-left-width").replace(
										"px", ""), 10)
						|| 0;
				var borderTop = parseInt(style
								.getPropertyValue("border-top-width").replace(
										"px", ""), 10)
						|| 0;
				var paddingLeft = parseInt(style
		...
		this.isScrolledToBottom = function(element) {
			...
			if (FBTrace.DBG_CONSOLE) {
				FBTrace.sysout("isScrolledToBottom offsetHeight: "
						+ element.offsetHeight + " onBottom:" + onBottom)
			}
			return onBottom
		};
		...
		this.scrollIntoCenterView = function(element, scrollBox, notX, notY) {
			...
			if (FBTrace.DBG_SOURCEFILES) {
				FBTrace.sysout("lib.scrollIntoCenterView ", "Element:"
								+ element.innerHTML)
			}
		};
		...
		this.copyTextStyles = function(fromNode, toNode, style) {
			...
			if (view) {
				if (!style) {
					style = this.isIE ? fromNode.currentStyle : view
							.getComputedStyle(fromNode, "")
				}
		...
		this.readBoxStyles = function(style) {
			...
			for (var styleName in styleNames) {
				styles[styleNames[styleName]] = parseInt(style
						.getPropertyCSSValue(styleName).cssText)
						|| 0
			}
				...
				FBTrace
						.sysout(
								"getInstanceForStyleSheet: "
										...
										+ " "
										+ (styleSheet.ownerNode && FBL
												.getElementXPath(styleSheet.ownerNode)),
								ownerDocument)
			}
			ownerDocument = ownerDocument
					|| FBL.getDocumentForStyleSheet(styleSheet);
			var ret = 0, styleSheets = ownerDocument.styleSheets, href = styleSheet.href;
			for (var i = 0; i < styleSheets.length; i++) {
				var curSheet = styleSheets[i];
				if (FBTrace.DBG_CSS) {
					FBTrace.sysout("getInstanceForStyleSheet: compare href "
							+ i
							+ " "
							+ curSheet.href
							+ " "
							...
							+ " "
							+ (curSheet.ownerNode && FBL
									.getElementXPath(curSheet.ownerNode)))
				}
		...
		this.createGlobalElement = function(tagName, properties) {
			properties = properties || {};
			var doc = FBL.Env.browser.document;
			var element = this.NS && doc.createElementNS ? doc.createElementNS(
					FBL.NS, tagName) : doc.createElement(tagName);
			for (var name in properties) {
				var propname = name;
		...
		this.isLeftClick = function(event) {
			return (this.isIE && event.type != "click"
					? event.button == 1
					: event.button == 0)
			...
			try {
				if (typeof listeners.length != "undefined") {
					if (FBTrace.DBG_DISPATCH) {
						FBTrace.sysout("FBL.dispatch", name + " to "
					...
					if (FBTrace.DBG_DISPATCH) {
						FBTrace.sysout("FBL.dispatch", name
										+ " to listeners of an object")
					}
		...
		this.restoreTextSelection = function(e) {
			if (typeof e.onselectstart != "undefined") {
				...
				if (!this.isFirefox) {
					this.removeEvent(e, "mousedown",
							disableTextSelectionHandler)
				}
			}
		};
		var eventTypes = {
			...
			drag : ["dragenter", "dragover", "dragexit", "dragdrop",
					"draggesture"],
			focus : ["focus", "blur"],
			form : ["submit", "reset", "change", "select", "input"],
			key : ["keydown", "keyup", "keypress"],
			load : ["load", "beforeunload", "unload", "abort", "error"],
			mouse : ["mousedown", "mouseup", "click", "dblclick", "mouseover",
					"mouseout", "mousemove"],
			mutation : ["DOMSubtreeModified", "DOMNodeInserted",
			...
			xul : ["popupshowing", "popupshown", "popuphiding", "popuphidden",
		...
		this.splitDataURL = function(url) {
			...
			if (props.hasOwnProperty("fileName")) {
				...
				if (props.hasOwnProperty("baseLineNumber")) {
					...
					var hint = decodeURIComponent(props.encodedContent.substr(
							0, 200)).replace(/\s*$/, "");
		...
		this.isSystemPage = function(win) {
			try {
				...
				if ((doc.styleSheets.length && doc.styleSheets[0].href == "chrome://global/content/xml/XMLPrettyPrint.css")
						|| (doc.styleSheets.length > 1 && doc.styleSheets[1].href == "chrome://browser/skin/feeds/subscribe.css")) {
					return true
				}
		...
		this.absoluteURL = function(url, baseURL) {
			return this.absoluteURLWithDots(url, baseURL).replace("/./", "/",
					"g")
		};
			...
			for (var i = 0; i < args.length; ++i) {
				try {
					...
					if (FBTrace.DBG_ERRORS) {
						...
						FBTrace.sysout("parseURLEncodedText EXCEPTION URI",
								args[i])
					}
				}
			}
		...
		this.parseJSONString = function(jsonString, originURL) {
			...
			try {
				...
				if (e.message.indexOf("is not defined")) {
					...
					try {
						jsonObject = Components.utils.evalInSandbox(jsonString,
								s)
					} catch (ex) {
		...
		this.isFunction = function(object) {
			...
			return toString.call(object) === "[object Function]" || this.isIE
		...
		this.instanceOf = function(object, className) {
			...
			if (object.ownerDocument) {
				var win = object.ownerDocument.defaultView
						|| object.ownerDocument.parentWindow;
				if (className in win) {
																				...
																				if (object instanceof SVGElement) {
																					...
																					if (object instanceof Element) {
																						...
																						if (object instanceof Text
																								|| object instanceof CDATASection) {
		...
		var domMemberMap = {};
		domMemberMap.Window = ["document", "frameElement", "innerWidth",
				"innerHeight", "outerWidth", "outerHeight", "screenX",
				"screenY", "pageXOffset", "pageYOffset", "scrollX", "scrollY",
				"scrollMaxX", "scrollMaxY", "status", "defaultStatus",
				"parent", "opener", "top", "window", "content", "self",
				"location", "history", "frames", "navigator", "screen",
				"menubar", "toolbar", "locationbar", "personalbar",
				"statusbar", "directories", "scrollbars", "fullScreen",
				"netscape", "java", "console", "Components", "controllers",
				"closed", "crypto", "pkcs11", "name", "property", "length",
				"sessionStorage", "globalStorage", "setTimeout", "setInterval",
				"clearTimeout", "clearInterval", "addEventListener",
				"removeEventListener", "dispatchEvent", "getComputedStyle",
				"captureEvents", "releaseEvents", "routeEvent",
				"enableExternalCapture", "disableExternalCapture", "moveTo",
				"moveBy", "resizeTo", "resizeBy", "scroll", "scrollTo",
				"scrollBy", "scrollByLines", "scrollByPages", "sizeToContent",
				"setResizable", "getSelection", "open", "openDialog", "close",
				"alert", "confirm", "prompt", "dump", "focus", "blur", "find",
				"back", "forward", "home", "stop", "print", "atob", "btoa",
				"updateCommands", "XPCNativeWrapper", "GeckoActiveXObject",
				"applicationCache"];
		domMemberMap.Location = ["href", "protocol", "host", "hostname",
				"port", "pathname", "search", "hash", "assign", "reload",
				"replace"];
		domMemberMap.Node = ["id", "className", "nodeType", "tagName",
				"nodeName", "localName", "prefix", "namespaceURI", "nodeValue",
				"ownerDocument", "parentNode", "offsetParent", "nextSibling",
				"previousSibling", "firstChild", "lastChild", "childNodes",
				"attributes", "dir", "baseURI", "textContent", "innerHTML",
				"addEventListener", "removeEventListener", "dispatchEvent",
				"cloneNode", "appendChild", "insertBefore", "replaceChild",
				"removeChild", "compareDocumentPosition", "hasAttributes",
				"hasChildNodes", "lookupNamespaceURI", "lookupPrefix",
				"normalize", "isDefaultNamespace", "isEqualNode", "isSameNode",
				"isSupported", "getFeature", "getUserData", "setUserData"];
		domMemberMap.Document = extendArray(domMemberMap.Node, [
						"documentElement", "body", "title", "location",
						"referrer", "cookie", "contentType", "lastModified",
						"characterSet", "inputEncoding", "xmlEncoding",
						"xmlStandalone", "xmlVersion", "strictErrorChecking",
						"documentURI", "URL", "defaultView", "doctype",
						"implementation", "styleSheets", "images", "links",
						"forms", "anchors", "embeds", "plugins", "applets",
						"width", "height", "designMode", "compatMode", "async",
						"preferredStylesheetSet", "alinkColor", "linkColor",
						"vlinkColor", "bgColor", "fgColor", "domain",
						"addEventListener", "removeEventListener",
						"dispatchEvent", "captureEvents", "releaseEvents",
						"routeEvent", "clear", "open", "close", "execCommand",
						"execCommandShowHelp", "getElementsByName",
						"getSelection", "queryCommandEnabled",
						"queryCommandIndeterm", "queryCommandState",
						"queryCommandSupported", "queryCommandText",
						"queryCommandValue", "write", "writeln", "adoptNode",
						"appendChild", "removeChild", "renameNode",
						...
						"domConfig", "evaluate", "evaluateFIXptr",
						"evaluateXPointer", "getAnonymousElementByAttribute",
						"getAnonymousNodes", "addBinding", "removeBinding",
						"getBindingParent", "getBoxObjectFor",
						"setBoxObjectFor", "getElementById",
						"getElementsByTagName", "getElementsByTagNameNS",
						"hasAttributes", "hasChildNodes", "importNode",
						"insertBefore", "isDefaultNamespace", "isEqualNode",
						"isSameNode", "isSupported", "load",
						"loadBindingDocument", "lookupNamespaceURI",
						"lookupPrefix", "normalize", "normalizeDocument",
						"getFeature", "getUserData", "setUserData"]);
		domMemberMap.Element = extendArray(domMemberMap.Node, ["clientWidth",
						"clientHeight", "offsetLeft", "offsetTop",
						"offsetWidth", "offsetHeight", "scrollLeft",
						"scrollTop", "scrollWidth", "scrollHeight", "style",
						"tabIndex", "title", "lang", "align", "spellcheck",
						"addEventListener", "removeEventListener",
						"dispatchEvent", "focus", "blur", "cloneNode",
						"appendChild", "insertBefore", "replaceChild",
						"removeChild", "compareDocumentPosition",
						"getElementsByTagName", "getElementsByTagNameNS",
						"getAttribute", "getAttributeNS", "getAttributeNode",
						"getAttributeNodeNS", "setAttribute", "setAttributeNS",
						"setAttributeNode", "setAttributeNodeNS",
						"removeAttribute", "removeAttributeNS",
						"removeAttributeNode", "hasAttribute",
						...
						"isSupported", "getFeature", "getUserData",
						"setUserData"]);
		domMemberMap.SVGElement = extendArray(domMemberMap.Element, ["x", "y",
						"width", "height", "rx", "ry", "transform", "href",
						"ownerSVGElement", "viewportElement",
						"farthestViewportElement", "nearestViewportElement",
						"getBBox", "getCTM", "getScreenCTM",
						"getTransformToElement", "getPresentationAttribute",
						"preserveAspectRatio"]);
		domMemberMap.SVGSVGElement = extendArray(domMemberMap.Element, ["x",
						"y", "width", "height", "rx", "ry", "transform",
						"viewBox", "viewport", "currentView", "useCurrentView",
						"pixelUnitToMillimeterX", "pixelUnitToMillimeterY",
						"screenPixelToMillimeterX", "screenPixelToMillimeterY",
						"currentScale", "currentTranslate", "zoomAndPan",
						"ownerSVGElement", "viewportElement",
						"farthestViewportElement", "nearestViewportElement",
						"contentScriptType", "contentStyleType", "getBBox",
						"getCTM", "getScreenCTM", "getTransformToElement",
						"getEnclosureList", "getIntersectionList",
						"getViewboxToViewportTransform",
						"getPresentationAttribute", "getElementById",
						"checkEnclosure", "checkIntersection",
						"createSVGAngle", "createSVGLength", "createSVGMatrix",
						"createSVGNumber", "createSVGPoint", "createSVGRect",
						"createSVGString", "createSVGTransform",
						"createSVGTransformFromMatrix", "deSelectAll",
						"preserveAspectRatio", "forceRedraw", "suspendRedraw",
						"unsuspendRedraw", "unsuspendRedrawAll",
						"getCurrentTime", "setCurrentTime", "animationsPaused",
						"pauseAnimations", "unpauseAnimations"]);
		domMemberMap.HTMLImageElement = extendArray(domMemberMap.Element, [
						"src", "naturalWidth", "naturalHeight", "width",
						"height", "x", "y", "name", "alt", "longDesc",
						"lowsrc", "border", "complete", "hspace", "vspace",
						"isMap", "useMap",]);
		domMemberMap.HTMLAnchorElement = extendArray(domMemberMap.Element, [
						"name", "target", "accessKey", "href", "protocol",
						"host", "hostname", "port", "pathname", "search",
						"hash", "hreflang", "coords", "shape", "text", "type",
						"rel", "rev", "charset"]);
		domMemberMap.HTMLIFrameElement = extendArray(domMemberMap.Element, [
						"contentDocument", "contentWindow", "frameBorder",
		...
		domMemberMap.HTMLTableElement = extendArray(domMemberMap.Element, [
						"bgColor", "border", "caption", "cellPadding",
						"cellSpacing", "frame", "rows", "rules", "summary",
						"tBodies", "tFoot", "tHead", "width", "createCaption",
						"createTFoot", "createTHead", "deleteCaption",
						"deleteRow", "deleteTFoot", "deleteTHead", "insertRow"]);
		domMemberMap.HTMLTableRowElement = extendArray(
				domMemberMap.Element,
				["bgColor", "cells", "ch", "chOff", "rowIndex",
						"sectionRowIndex", "vAlign", "deleteCell", "insertCell"]);
		domMemberMap.HTMLTableCellElement = extendArray(domMemberMap.Element, [
						"abbr", "axis", "bgColor", "cellIndex", "ch", "chOff",
		...
		domMemberMap.HTMLButtonElement = extendArray(domMemberMap.Element, [
						"accessKey", "disabled", "form", "name", "type",
						"value", "click"]);
		domMemberMap.HTMLInputElement = extendArray(domMemberMap.Element, [
						"type", "value", "checked", "accept", "accessKey",
						"alt", "controllers", "defaultChecked", "defaultValue",
						"disabled", "form", "maxLength", "name", "readOnly",
						"selectionEnd", "selectionStart", "size", "src",
						"textLength", "useMap", "click", "select",
						"setSelectionRange"]);
		domMemberMap.HTMLFormElement = extendArray(domMemberMap.Element, [
						"acceptCharset", "action", "author", "elements",
						"encoding", "enctype", "entry_id", "length", "method",
						"name", "post", "target", "text", "url", "reset",
						"submit"]);
		...
		domMemberMap.HTMLHtmlElement = extendArray(domMemberMap.Element,
				["version"]);
		domMemberMap.Text = extendArray(domMemberMap.Node, ["data", "length",
						"appendData", "deleteData", "insertData",
						"replaceData", "splitText", "substringData"]);
		domMemberMap.Attr = extendArray(domMemberMap.Node, ["name", "value",
						"specified", "ownerElement"]);
		domMemberMap.Event = ["type", "target", "currentTarget",
				"originalTarget", "explicitOriginalTarget", "relatedTarget",
				"rangeParent", "rangeOffset", "view", "keyCode", "charCode",
				"screenX", "screenY", "clientX", "clientY", "layerX", "layerY",
				"pageX", "pageY", "detail", "button", "which", "ctrlKey",
				"shiftKey", "altKey", "metaKey", "eventPhase", "timeStamp",
				"bubbles", "cancelable", "cancelBubble", "isTrusted", "isChar",
				"getPreventDefault", "initEvent", "initMouseEvent",
				"initKeyEvent", "initUIEvent", "preventBubble",
				"preventCapture", "preventDefault", "stopPropagation"];
		this.domConstantMap = {
			ELEMENT_NODE : 1,
			ATTRIBUTE_NODE : 1,
			TEXT_NODE : 1,
		...
		this.cssInfo = {
			background : ["bgRepeat", "bgAttachment", "bgPosition", "color",
					"systemColor", "none"],
			...
			border : ["borderStyle", "thickness", "color", "systemColor",
					"none"],
		...
		this.cssKeywords = {
			appearance : ["button", "button-small", "checkbox",
					"checkbox-container", "checkbox-small", "dialog",
					"listbox", "menuitem", "menulist", "menulist-button",
					"menulist-textfield", "menupopup", "progressbar", "radio",
					"radio-container", "radio-small", "resizer", "scrollbar",
					...
					"separator", "statusbar", "tab", "tab-left-edge",
					"tabpanels", "textfield", "toolbar", "toolbarbutton",
					"toolbox", "tooltip", "treeheadercell",
					"treeheadersortarrow", "treeitem", "treetwisty",
					"treetwistyopen", "treeview", "window"],
			systemColor : ["ActiveBorder", "ActiveCaption", "AppWorkspace",
					...
					"WindowFrame", "WindowText", "-moz-field",
					"-moz-fieldtext", "-moz-workspace",
					"-moz-visitedhyperlinktext", "-moz-use-text-color"],
			color : ["AliceBlue", "AntiqueWhite", "Aqua", "Aquamarine",
					...
					"DarkViolet", "DeepPink", "DarkSkyBlue", "DimGray",
					...
					"Tomato", "Turquoise", "Violet", "VioletRed", "Wheat",
			...
			cursor : ["auto", "cell", "context-menu", "crosshair", "default",
					"help", "pointer", "progress", "move", "e-resize",
					"all-scroll", "ne-resize", "nw-resize", "n-resize",
					"se-resize", "sw-resize", "s-resize", "w-resize",
					"ew-resize", "ns-resize", "nesw-resize", "nwse-resize",
					"col-resize", "row-resize", "text", "vertical-text",
					"wait", "alias", "copy", "move", "no-drop", "not-allowed",
					"-moz-alias", "-moz-cell", "-moz-copy", "-moz-grab",
					"-moz-grabbing", "-moz-contextmenu", "-moz-zoom-in",
					"-moz-zoom-out", "-moz-spinning"],
			direction : ["ltr", "rtl"],
			bgAttachment : ["scroll", "fixed"],
			bgPosition : ["top", "center", "bottom", "left", "right"],
			bgRepeat : ["repeat", "repeat-x", "repeat-y", "no-repeat"],
			borderStyle : ["hidden", "dotted", "dashed", "solid", "double",
					"groove", "ridge", "inset", "outset", "-moz-bg-inset",
					"-moz-bg-outset", "-moz-bg-solid"],
			borderCollapse : ["collapse", "separate"],
			overflow : ["visible", "hidden", "scroll",
					"-moz-scrollbars-horizontal", "-moz-scrollbars-none",
			...
			listStyleType : ["disc", "circle", "square", "decimal",
					...
					"hiragana-iroha", "katakana-iroha", "inherit"],
			...
			fontSize : ["xx-small", "x-small", "small", "medium", "large",
					"x-large", "xx-large", "smaller", "larger"],
			fontFamily : ["Arial", "Comic Sans MS", "Georgia", "Tahoma",
					"Verdana", "Times New Roman", "Trebuchet MS",
					"Lucida Grande", "Helvetica", "serif", "sans-serif",
					"cursive", "fantasy", "monospace", "caption", "icon",
					"menu", "message-box", "small-caption", "status-bar",
					"inherit"],
			display : ["block", "inline", "inline-block", "list-item",
					"marker", "run-in", "compact", "table", "inline-table",
					"table-row-group", "table-column", "table-column-group",
					"table-header-group", "table-footer-group", "table-row",
					"table-cell", "table-caption", "-moz-box", "-moz-compact",
					"-moz-deck", "-moz-grid", "-moz-grid-group",
					"-moz-grid-line", "-moz-groupbox", "-moz-inline-block",
					"-moz-inline-box", "-moz-inline-grid", "-moz-inline-stack",
					"-moz-inline-table", "-moz-marker", "-moz-popup",
					"-moz-runin", "-moz-stack"],
			...
			verticalAlign : ["baseline", "sub", "super", "top", "text-top",
		...
		this.Ajax = {
			requests : [],
			transport : null,
			states : ["Uninitialized", "Loading", "Loaded", "Interactive",
					"Complete"],
			initialize : function() {
			...
			onStateChange : function(options) {
				...
				if (state == "Complete") {
					var success = t.status == 200, response = this
							.getResponse(o);
					if (fn = o.onUpdate) {
						fn(response, o)
					}
	...
	FBL.ns(function() {
		with (FBL) {
			...
			window.Firebug = FBL.Firebug = {
				...
				initialize : function() {
					if (FBTrace.DBG_INITIALIZE) {
						FBTrace.sysout("Firebug.initialize",
				...
				getRep : function(object) {
					...
					for (var i = 0; i < reps.length; ++i) {
						...
						try {
							...
							if (FBTrace.DBG_ERRORS) {
								FBTrace.sysout("firebug.getRep FAILS: ",
										exc.message || exc);
								FBTrace.sysout("firebug.getRep reps[" + i + "/"
										+ reps.length + "]: Rep="
										+ reps[i].className)
							}
						}
			...
			if (!Env.Options.enablePersistent || Env.Options.enablePersistent
					&& Env.isChromeContext || Env.isDebugMode) {
			...
			Firebug.Panel = {
				...
				create : function(context, doc) {
					this.hasSidePanel = parentPanelMap
					...
					var options = this.options = extend(Firebug.Panel.options,
							this.options);
				...
				showToolbarButtons : function(buttonsId, show) {
					try {
						...
						if (FBTrace.DBG_ERRORS) {
							...
							if (!this.context.browser) {
								FBTrace
										.dumpStack("firebug.Panel showToolbarButtons no browser")
							}
						}
					}
				},
				...
				select : function(object, forceUpdate) {
					...
					if (FBTrace.DBG_PANELS) {
						FBTrace.sysout("firebug.select " + this.name
								+ " forceUpdate: " + forceUpdate + " " + object
				...
				markChange : function(skipSelf) {
					if (this.dependents) {
						if (skipSelf) {
							...
							this.context.invalidatePanels.apply(this.context,
									this.dependents)
						}
					}
				},
			...
			Firebug.MeasureBox = {
				startMeasuring : function(target) {
					if (!this.measureBox) {
						this.measureBox = target.ownerDocument
								.createElement("span");
				...
				measureInputText : function(value) {
					...
					if (!Firebug.showTextNodesWithWhitespace) {
						value = value.replace(/\t/g, "mmmmmm").replace(/\ /g,
								"m")
					}
					this.measureBox.innerHTML = value;
					return {
				...
				getBox : function(target) {
					var style = this.measureBox.ownerDocument.defaultView
							.getComputedStyle(this.measureBox, "");
		...
		with (FBL) {
			...
			FBL.PanelBar = {
				...
				create : function(ownerPanel) {
					...
					if (ownerPanel) {
						...
						ownerPanel.sidePanelBarBoxNode
								.appendChild(ownerPanel.sidePanelBarNode)
					}
					var panels = Firebug.panelTypes;
					for (var i = 0, p; p = panels[i]; i++) {
						if (!ownerPanel && !p.prototype.parentPanel
					...
					for (var name in this.panelMap) {
						(function(self, name) {
							...
							Firebug.chrome.addController([
									self.panelMap[name].tabNode, "mousedown",
									onTabClick])
						})(this, name)
					}
				},
			...
			Button.prototype = extend(Controller, {
				...
				initialize : function() {
					...
					this
							.addController([element, "mousedown",
									this.handlePress]);
					if (this.type == "normal") {
						this.addController([element, "mouseup",
										this.handleUnpress], [element,
										"mouseout", this.handleUnpress], [
			...
			FBL.Menu = function(options) {
				if (!options.element) {
					...
					options.element = MenuPlate.tag.append({
								object : options
							}, getElementByClass(Firebug.chrome.document,
									"fbBody"), MenuPlate)
				}
				append(this, options);
				...
				this.handleWindowMouseDown = bind(this.handleWindowMouseDown,
						this)
			};
			var menuMap = {};
			Menu.prototype = extend(Controller, {
				...
				initialize : function() {
					...
					this.addController([this.element, "mousedown",
									this.handleMouseDown], [this.element,
				...
				show : function(x, y) {
					...
					if (this.parentMenu) {
						...
						addEvent(Firebug.chrome.document, "mousedown",
					...
					x = Math.max(x, 0);
					y = Math.min(y, size.height - this.element.clientHeight
									- 10);
					y = Math.max(y, 0);
				...
				showChildMenu : function(target) {
					...
					this.showChildTimeout = Firebug.chrome.window.setTimeout(
							function() {
								...
								var childMenuObject = menuMap
										.hasOwnProperty(id) ? menuMap[id] : {
									element : $(id)
								};
								var childMenu = new Menu(extend(
										childMenuObject, {
											parentMenu : parent,
											parentTarget : target
										}));
								var offsetLeft = isIE6 ? -1 : -6;
								childMenu.show(box.left + box.width
												+ offsetLeft, box.top - 6);
				...
				clearShowChildTimeout : function() {
					if (this.showChildTimeout) {
						Firebug.chrome.window
								.clearTimeout(this.showChildTimeout);
					...
					if (target && !hasClass(target, "fbMenuDisabled")) {
						...
						if (type == "checkbox") {
							...
							if (isFunction(this.onCheck)) {
								this.onCheck.call(this, target, value,
										!wasChecked)
							}
						}
						if (type == "radiobutton") {
							var selectedRadios = getElementsByClass(
									target.parentNode, "fbMenuRadioSelected");
				...
				handleWindowMouseDown : function(event) {
					...
					if (!target) {
						removeEvent(Firebug.chrome.document, "mousedown",
								this.handleWindowMouseDown);
						this.hide()
					}
				},
				handleMouseOver : function(event) {
					...
					if (childMenu) {
						removeClass(childMenu.parentTarget,
								"fbMenuGroupSelected");
						if (childMenu.parentTarget != target
								&& childMenu.isVisible) {
							childMenu.clearHideTimeout();
							childMenu.hideTimeout = Firebug.chrome.window
									.setTimeout(function() {
												childMenu.destroy()
											}, 300)
						}
	...
	FBL.ns(function() {
		with (FBL) {
			...
			var shouldFixElementFromPoint = isOpera || isSafari
					&& browserVersion < "532";
			...
			FBL.Context = function(win) {
				...
				this.eval = this.window
						.eval("new Function('try{ return window.eval.apply(window,arguments) }catch(E){ E."
								+ evalError + "=true; return E }')")
			};
			FBL.Context.prototype = {
				...
				setTimeout : function(fn, delay) {
					...
					var timeout = win.setTimeout.apply ? win.setTimeout.apply(
							win, arguments) : win.setTimeout(fn, delay);
					if (!this.timeouts) {
						this.timeouts = {}
					}
				...
				setInterval : function(fn, delay) {
					var win = this.window;
					var timeout = win.setInterval.apply ? win.setInterval
							.apply(win, arguments) : win.setInterval(fn, delay);
					if (!this.intervals) {
						this.intervals = {}
					}
					...
					if (context == "window") {
						...
						cmd = api
								? "(function(arguments){ with(" + api
										+ "){ return (" + expr + ") } }).call("
										+ context + ",undefined)"
								: "(function(arguments){ return (" + expr
										+ ") }).call(" + context
										+ ",undefined)";
						result = this.eval(cmd);
						if (result && result[evalError]) {
				...
				getWindowSize : function() {
					...
					if (typeof this.window.innerWidth == "number") {
						...
						if ((el = this.document.documentElement)
				...
				getElementFromPoint : function(x, y) {
					if (shouldFixElementFromPoint) {
						...
						return this.document.elementFromPoint(x + scroll.left,
								y + scroll.top)
					} else {
				...
				getMeasurementBox1 : function(el, name) {
					...
					for (var i = 0, sufix; sufix = sufixes[i]; i++) {
						result[i] = Math.round(this.getMeasurementInPixels(el,
								name + sufix))
					}
					return {
						top : result[0],
				...
				getMeasurementBox : function(el, name) {
					var result = [];
					var sufixes = name == "border" ? ["TopWidth", "LeftWidth",
							"BottomWidth", "RightWidth"] : ["Top", "Left",
							"Bottom", "Right"];
					if (isIE) {
						...
						for (var i = 0, sufix; sufix = sufixes[i]; i++) {
							propName = name + sufix;
							cssValue = el.currentStyle[propName]
									|| el.style[propName];
							if (cssValue == "auto") {
								if (!autoMargin) {
								...
								result[i] = this.getMeasurementInPixels(el,
										propName)
							}
						}
					} else {
						for (var i = 0, sufix; sufix = sufixes[i]; i++) {
							result[i] = this.getMeasurementInPixels(el, name
											+ sufix)
						}
					}
					return {
				...
				getCSSAutoMarginBox : function(el) {
					...
					var marginBottom = box.offsetHeight - clone.offsetHeight
							- 2 - marginTop;
					var marginLeft = clone.offsetLeft - box.offsetLeft - 1;
					var marginRight = box.offsetWidth - clone.offsetWidth - 2
							- marginLeft;
	...
	FBL.ns(function() {
		with (FBL) {
			var WindowDefaultOptions = {
				...
				id : "FirebugUI",
				height : 250
			}, commandLine, fbTop, fbContent, fbContentStyle, fbBottom, fbBtnInspect, fbToolbar, fbPanelBox1, fbPanelBox1Style, fbPanelBox2, fbPanelBox2Style, fbPanelBar2Box, fbPanelBar2BoxStyle, fbHSplitter, fbVSplitter, fbVSplitterStyle, fbPanel1, fbPanel1Style, fbPanel2, fbPanel2Style, fbConsole, fbConsoleStyle, fbHTML, fbCommandLine, fbLargeCommandLine, fbLargeCommandButtons, topHeight, topPartialHeight, chromeRedrawSkipRate = isIE
					? 75
					: isOpera ? 80 : 75, lastSelectedPanelName, focusCommandLineState = 0, lastFocusedPanelName, lastHSplitterMouseMove = 0, onHSplitterMouseMoveBuffer = null, onHSplitterMouseMoveTimer = null, lastVSplitterMouseMove = 0;
			FBL.FirebugChrome = {
				isOpen : false,
				height : 250,
				sidePanelWidth : 350,
				...
				create : function() {
					if (FBTrace.DBG_INITIALIZE) {
						FBTrace.sysout("FirebugChrome.create",
				...
				initialize : function() {
					if (FBTrace.DBG_INITIALIZE) {
						FBTrace.sysout("FirebugChrome.initialize",
			...
			var createChromeWindow = function(options) {
				...
				var chrome = {}, context = options.context || Env.browser, type = chrome.type = Env.Options.enablePersistent
						? "popup"
						: options.type, isChromeFrame = type == "frame", useLocalSkin = Env.useLocalSkin, url = useLocalSkin
						? Env.Location.skin
						: "about:blank", body = context.document
						.getElementsByTagName("body")[0], formatNode = function(
						node) {
					if (!Env.isDebugMode) {
					...
					node.style.position = noFixedPosition
							? "absolute"
							: "fixed";
					...
					if (style.styleSheet) {
						...
						style.appendChild(context.document
				...
				try {
					if (type == "div") {
						...
						if (isChromeFrame) {
							...
							var height = FirebugChrome.height || options.height, options = [
									"true,top=",
									Math.max(screen.availHeight - height - 61,
											0), ",left=0,height=", height,
									",width=", screen.availWidth - 10,
									",resizable"].join(""), node = chrome.node = context.window
									.open(url, "popup", options);
							if (node) {
								try {
									node.focus()
								} catch (E) {
					...
					if (!useLocalSkin) {
						var tpl = getChromeTemplate(!isChromeFrame), doc = isChromeFrame
								? node.contentWindow.document
								: node.document;
						doc.write(tpl);
						doc.close()
					}
					var win, waitDelay = useLocalSkin ? isChromeFrame
							? 200
							: 300 : 100, waitForWindow = function() {
						if (isChromeFrame
								&& (win = node.contentWindow)
								&& node.contentWindow.document
										.getElementById("fbCommandLine")
								|| !isChromeFrame
								&& (win = node.window)
								&& node.document
								&& node.document
			...
			var onChromeLoad = function onChromeLoad(chrome) {
				Env.chrome = chrome;
				if (FBTrace.DBG_INITIALIZE) {
					FBTrace.sysout("Chrome onChromeLoad",
				...
				if (Env.Options.enablePersistent) {
					...
					if (chrome.type == "frame" || chrome.type == "div") {
						...
						if (chrome.type == "popup") {
							...
							dispatch(newChrome.panelMap, "detach", [oldChrome,
											newChrome]);
							if (oldChrome) {
								oldChrome.close()
							}
			...
			append(ChromeBase, {
				...
				testMenu : function() {
					var firebugMenu = new Menu({
						...
						visitDiscussionGroup : function() {
							this
									.visit("http://groups.google.com/group/firebug")
						},
						visitIssueTracker : function() {
							this
									.visit("http://code.google.com/p/fbug/issues/list")
						},
						visit : function(url) {
							window.open(url)
						}
					...
					var firebugOptionsMenu = {
						...
						updateMenu : function(target) {
							var options = getElementsByClass(target.parentNode,
									"fbMenuOption");
					...
					var testMenuClick = function(event) {
						...
						if (menu.isVisible) {
							...
							var offsetLeft = isIE6 ? 1 : -4, chrome = Firebug.chrome, box = chrome
									.getElementBox(target), offset = chrome.type == "div"
									? chrome.getElementPosition(chrome.node)
									: {
										top : 0,
										left : 0
									};
							menu.show(box.left + offsetLeft - offset.left,
				...
				initialize : function() {
					if (Env.bookmarletOutdated) {
						Firebug.Console
								.logFormatted(
										["A new bookmarlet version is available. Please visit http://getfirebug.com/firebuglite and update it."],
					...
					if (FBTrace.DBG_INITIALIZE) {
						FBTrace.sysout("Firebug.chrome.initialize",
					...
					this.addController([$("fbLargeCommandLineIcon"), "click",
							this.showLargeCommandLine]);
					var self = this;
					setTimeout(function() {
								self
										.selectPanel(FirebugChrome.selectedPanelName);
					...
					var onPanelMouseDown = function onPanelMouseDown(event) {
						...
						if (FBL.isLeftClick(event)) {
							var editable = FBL.getAncestorByClass(target,
									"editable");
							if (editable) {
					...
					Firebug.chrome.keyCodeListen = function(key, filter,
							listener, capture) {
						if (!filter) {
					...
					Firebug.chrome.keyIgnore = function(listener) {
						removeEvent(Firebug.chrome.document, "keydown",
								listener[0])
					};
					this.addController(
				...
				toggle : function(forceOpen, popup) {
					if (popup) {
						...
						if (isOpera && Firebug.chrome.type == "popup"
				...
				draw : function() {
					...
					var commandLineHeight = Firebug.chrome.commandLineVisible
							...
							: 0, y = Math.max(size.height, topHeight), heightValue = Math
							.max(y - topHeight - commandLineHeight, 0), height = heightValue
							+ "px", sideWidthValue = Firebug.chrome.sidePanelVisible
					...
					if (isIE || isOpera) {
						fbVSplitterStyle.height = Math.max(y - topPartialHeight
										- commandLineHeight, 0)
								+ "px"
					}
					...
					if (Firebug.chrome.sidePanelVisible) {
						...
						if (Firebug.chrome.largeCommandLineVisible) {
							fbLargeCommandLine = $("fbLargeCommandLine");
							fbLargeCommandLine.style.height = heightValue - 4
									+ "px";
							fbLargeCommandLine.style.width = sideWidthValue - 2
									+ "px";
							fbLargeCommandButtons = $("fbLargeCommandButtons");
			...
			var ChromeFrameBase = extend(ChromeBase, {
				...
				initialize : function() {
					...
					this.addController([Firebug.browser.window, "resize",
									this.resize], [$("fbWindow_btClose"),
									"click", this.close], [
									$("fbWindow_btDetach"), "click",
									this.detach], [$("fbWindow_btDeactivate"),
									"click", this.deactivate]);
					if (!Env.Options.enablePersistent) {
						this.addController([Firebug.browser.window, "unload",
								Firebug.shutdown])
					}
					if (noFixedPosition) {
						this.addController([Firebug.browser.window, "scroll",
								this.fixIEPosition])
					}
				...
				fixIEPosition : function() {
					...
					var offset = isIE ? doc.body.clientTop
					...
					this.node.style.top = maxHeight - height + scroll.top
							+ "px";
					...
					if (fbVSplitterStyle) {
						fbVSplitterStyle.right = FirebugChrome.sidePanelWidth
								+ "px"
					}
					this.draw()
				}
			...
			var ChromeMini = extend(Controller, {
				...
				initialize : function() {
					...
					this.addController([$("fbMiniIcon", doc), "click",
							onMiniIconClick]);
					if (noFixedPosition) {
						this.addController([Firebug.browser.window, "scroll",
								this.fixIEPosition])
			...
			var ChromePopupBase = extend(ChromeBase, {
				initialize : function() {
					...
					ChromeBase.initialize.call(this);
					this.addController([Firebug.chrome.window, "resize",
									this.resize], [Firebug.chrome.window,
									"unload", this.destroy]);
					if (Env.Options.enablePersistent) {
				...
				destroy : function() {
					...
					if (Env.Options.enablePersistent) {
						removeEvent(Firebug.browser.window, "unload",
								this.persist)
					}
					...
					var waitMainWindow = function() {
						...
						try {
							if (window.opener
								...
								try {
									var persistDelay = new Date().getTime()
											- persistTimeStart;
									...
									registerConsole();
									var chrome = Firebug.chrome;
									addEvent(Firebug.browser.window, "unload",
											chrome.persist);
									FBL.cacheDocument();
									Firebug.Inspector.create();
									...
									htmlPanel.createUI();
									Firebug.Console
											.info("Firebug could not capture console calls during "
													+ persistDelay + "ms")
								} catch (pE) {
									alert("persist error: "
											+ (pE.message || pE))
			...
			var changeCommandLineVisibility = function changeCommandLineVisibility(
				...
				var visible = Firebug.chrome.commandLineVisible = typeof visibility == "boolean"
						? visibility
			...
			var changeSidePanelVisibility = function changeSidePanelVisibility(
				...
				Firebug.chrome.sidePanelVisible = typeof visibility == "boolean"
						? visibility
			...
			var onHSplitterMouseDown = function onHSplitterMouseDown(event) {
				...
				if (isIE) {
					addEvent(Firebug.browser.document.documentElement,
							"mouseleave", onHSplitterMouseUp)
				}
			...
			var onHSplitterMouseMove = function onHSplitterMouseMove(event) {
				...
				if (win != win.parent) {
					...
					if (frameElement) {
						var framePos = Firebug.browser
								.getElementPosition(frameElement).top;
						clientY += framePos;
						if (frameElement.style.position != "fixed") {
							clientY -= Firebug.browser
									.getWindowScrollPosition().top
						}
					}
				}
				if (isOpera && isQuiksMode
						&& win.frameElement.id == "FirebugUI") {
					clientY = Firebug.browser.getWindowSize().height
							- win.frameElement.offsetHeight + clientY
				}
				...
				if (new Date().getTime() - lastHSplitterMouseMove > chromeRedrawSkipRate) {
					...
					if (!onHSplitterMouseMoveTimer) {
						onHSplitterMouseMoveTimer = setTimeout(
								handleHSplitterMouseMove, chromeRedrawSkipRate)
					}
				}
				cancelEvent(event, true);
				return false
			...
			var handleHSplitterMouseMove = function() {
				...
				var scrollbarSize = !isIE
						&& (scrollSize.width > windowSize.width) ? 17 : 0;
				var height = windowSize.height;
				var chromeHeight = Math.max(height - clientY + 5
								- scrollbarSize, fixedHeight);
				chromeHeight = Math.min(chromeHeight, windowSize.height
								- scrollbarSize);
			...
			var onHSplitterMouseUp = function onHSplitterMouseUp(event) {
				...
				if (isIE) {
					removeEvent(Firebug.browser.document.documentElement,
							"mouseleave", onHSplitterMouseUp)
				}
				...
				if (new Date().getTime() - lastVSplitterMouseMove > chromeRedrawSkipRate) {
					...
					if (target && target.ownerDocument) {
						...
						var win = document.all
								...
								: event.target.ownerDocument.defaultView;
						if (win != win.parent) {
							clientX += win.frameElement
									? win.frameElement.offsetLeft
									: 0
						}
						var size = Firebug.chrome.getSize();
	...
	FBL.ns(function() {
		with (FBL) {
			...
			Sizzle.filter = function(expr, set, inplace, not) {
				var old = expr, result = [], curLoop = set, match, anyFound, isXMLFilter = set
						&& set[0] && isXML(set[0]);
			...
			if (document.documentElement.compareDocumentPosition) {
				sortOrder = function(a, b) {
					...
					var ret = a.compareDocumentPosition(b) & 4 ? -1 : a === b
							? 0
							: 1;
					if (ret === 0) {
						hasDuplicate = true
	...
	(function	() {
			...
			var isXML = function(elem) {
				return elem.nodeType === 9
						&& elem.documentElement.nodeName !== "HTML"
						|| !!elem.ownerDocument
						&& elem.ownerDocument.documentElement.nodeName !== "HTML"
			};
			var posProcess = function(selector, context) {
				var tmpSet = [], later = "", match, root = context.nodeType
						? [context]
						: context;
	...
	(function() {
		...
		var domplate = FBL.domplate = function() {
			var lastSubject;
			for (var i = 0; i < arguments.length; ++i) {
				lastSubject = lastSubject ? copyObject(lastSubject,
		...
		DomplateTag.prototype = {
			merge : function(args, oldTag) {
				...
				if (args.length) {
					parseChildren(args, hasAttrs ? 1 : 0, this.vars,
							this.children)
				}
				if (hasAttrs) {
					...
					if (name.indexOf("on") == 0) {
						...
						if (name.indexOf("_") == 0) {
							...
							if (name.indexOf("$") == 0) {
								...
								if (name == "class"
										&& this.attrs.hasOwnProperty(name)) {
									this.attrs[name] += " " + val
								} else {
									this.attrs[name] = val
								}
			...
			compileMarkup : function() {
				this.markupArgs = [];
				var topBlock = [], topOuts = [], blocks = [], info = {
			...
			generateChildMarkup : function(topBlock, topOuts, blocks, info) {
				for (var i = 0; i < this.children.length; ++i) {
					var child = this.children[i];
					if (isTag(child)) {
						child.tag.generateMarkup(topBlock, topOuts, blocks,
								info)
					} else {
			...
			generateDOM : function(path, blocks, args) {
				...
				if (this.listeners) {
					for (var i = 0; i < this.listeners.length; i += 2) {
						...
						blocks.push('addEvent(node, "', this.listeners[i],
		...
		DomplateEmbed.prototype = copyObject(DomplateTag.prototype, {
					...
					generateDOM : function(path, blocks, args) {
						...
						blocks.push(embedName + " = __link__(node, ",
								valueName, ", ", argsName, ");");
						return embedName
					}
				});
		DomplateLoop.prototype = copyObject(DomplateTag.prototype, {
					merge : function(args, oldTag) {
						...
						this.vars = [];
						this.children = oldTag
								? copyArray(oldTag.children)
								: [];
					...
					generateMarkup : function(topBlock, topOuts, blocks, info) {
						...
						blocks.push("__loop__.apply(this, [", iterName,
								", __out__, function(", this.varName,
								", __out__) {");
						this.generateChildMarkup(topBlock, topOuts, blocks,
								info);
						this.addCode(topBlock, topOuts, blocks);
						blocks.push("}]);")
					},
		...
		function parseParts(str) {
			...
			var m;
			while (m = re.exec(str)) {
				var pre = str.substr(index, (re.lastIndex - m[0].length)
								- index);
				if (pre) {
					parts.push(pre)
				}
		...
		var Renderer = {
			renderHTML : function(args, outputs, self) {
				...
				this.tag.renderMarkup.apply(self ? self : this.tag.subject,
						markupArgs);
				return code.join("")
			},
			...
			insertBefore : function(args, before, self) {
				return this.insertNode(args, before.ownerDocument, before,
						false, self)
			},
			insertAfter : function(args, after, self) {
				return this.insertNode(args, after.ownerDocument, after, true,
						self)
			},
				...
				if (isAfter) {
					while (womb.firstChild) {
						if (element.nextSibling) {
							element.parentNode.insertBefore(womb.firstChild,
									element.nextSibling)
						} else {
					...
					while (womb.lastChild) {
						element.parentNode
								.insertBefore(womb.lastChild, element)
					}
				}
		...
		function defineTags() {
			for (var i = 0; i < arguments.length; ++i) {
				...
				var fn = new Function("var newTag = new arguments.callee.DomplateTag('"
						+ tagName + "'); return newTag.merge(arguments);");
				fn.DomplateTag = DomplateTag;
				var fnName = tagName.toUpperCase();
				FBL[fnName] = fn
			}
		}
		defineTags("a", "button", "br", "canvas", "code", "col", "colgroup",
				"div", "fieldset", "form", "h1", "h2", "h3", "hr", "img",
				"input", "label", "legend", "li", "ol", "optgroup", "option",
				"p", "pre", "select", "span", "strong", "table", "tbody", "td",
				"textarea", "tfoot", "th", "thead", "tr", "tt", "ul", "iframe")
	})();
		...
		with (FBL) {
			...
			this.Func = domplate(Firebug.Rep, {
				...
				getTooltip : function(fn, context) {
					...
					if (script) {
						return $STRF("Line", [normalizeURL(script.fileName),
				...
				getContextMenuItems : function(fn, target, context, script) {
					...
					var scriptInfo = getSourceFileAndLineByScript(context,
							script);
					var monitored = scriptInfo
							? fbs.isMonitored(scriptInfo.sourceFile.href,
									scriptInfo.lineNo)
							: false;
					var name = script
							? getFunctionName(script, context)
							: fn.name;
					return [{
								label : "CopySource",
								...
								type : "checkbox",
								checked : monitored,
								command : bindFixed(this.monitor, this, fn,
										script, monitored)
							}]
				}
			});
			this.Obj = domplate(Firebug.Rep, {
						tag : OBJECTLINK(SPAN({
											"class" : "objectTitle"
										}, "$object|getTitle"), FOR("prop",
										"$object|propIterator", " $prop.name=",
										SPAN({
													"class" : "objectPropValue"
												}, "$prop.value|cropString"))),
							...
							try {
								for (var name in object) {
									...
									if (t == "boolean"
											...
											|| (t == "string" && val)
											|| (t == "object"
													&& !isFunction(val) && val && val.toString)) {
										var title = (t == "object")
												? Firebug.getRep(val)
														.getTitle(val)
												: val + "";
			...
			this.Arr = domplate(Firebug.Rep, {
				...
				shortTag : OBJECTBOX({
							_repObject : "$object"
								...
								"$object|shortArrayIterator", TAG("$item.tag",
										{
											object : "$item.object"
										}), SPAN({
											"class" : "arrayComma",
			...
			this.Element = domplate(Firebug.Rep, {
				tag : OBJECTLINK("&lt;", SPAN({
									"class" : "nodeTag"
								}, "$object.nodeName|toLowerCase"), FOR("attr",
								"$object|attrIterator",
								"&nbsp;$attr.nodeName=&quot;", SPAN({
											"class" : "nodeValue"
										}, "$attr.nodeValue"), "&quot;"),
				...
				attrIterator : function(elt) {
					...
					if (elt.attributes) {
						for (var i = 0; i < elt.attributes.length; ++i) {
							...
							if (attr.nodeName
									&& attr.nodeName.indexOf("firebug-") != -1) {
								continue
							} else {
								if (attr.nodeName == "id") {
									idAttr = attr
				...
				persistor : function(context, xpath) {
					var elts = xpath ? getElementsByXPath(
							context.window.document, xpath) : null;
				...
				getContextMenuItems : function(elt, target, context) {
					...
					return [{
								...
								label : "CopyInnerHTML",
								command : bindFixed(this.copyInnerHTML, this,
										elt)
							}, {
								label : "CopyXPath",
								...
								type : "checkbox",
								checked : monitored,
								command : bindFixed(toggleMonitorEvents, FBL,
										elt, null, monitored, context)
							}, "-", {
			...
			this.StyleSheet = domplate(Firebug.Rep, {
						...
						getContextMenuItems : function(styleSheet, target,
								context) {
							return [{
								label : "CopyLocation",
								command : bindFixed(this.copyURL, this,
										styleSheet)
							}, "-", {
								label : "OpenInTab",
								command : bindFixed(this.openInTab, this,
										styleSheet)
							}]
						},
			...
			this.Window = domplate(Firebug.Rep, {
						...
						getLocation : function(win) {
							try {
								...
								if (FBTrace.DBG_ERRORS) {
									FBTrace
											.sysout("reps.Window window closed?")
								}
							}
						},
						className : "object",
			...
			this.Event = domplate(Firebug.Rep, {
						...
						summarizeEvent : function(event) {
							...
							if (eventFamily == "mouse") {
								info.push("clientX=", event.clientX,
										", clientY=", event.clientY)
							} else {
								if (eventFamily == "key") {
									info.push("charCode=", event.charCode,
						...
						className : "object",
						supportsObject : function(object) {
							return instanceOf(object, "Event")
									|| instanceOf(object, "EventCopy")
						},
			...
			this.SourceLink = domplate(Firebug.Rep, {
						...
						hideSourceLink : function(sourceLink) {
							return sourceLink
									? sourceLink.href
											.indexOf("XPCSafeJSObjectWrapper") != -1
									: true
						},
						...
						inspectObject : function(sourceLink, context) {
							if (sourceLink.type == "js") {
								var scriptFile = getSourceFileByHref(
										sourceLink.href, context);
								if (scriptFile) {
						...
						getContextMenuItems : function(sourceLink, target,
								context) {
							return [{
								label : "CopyLocation",
								command : bindFixed(this.copyLink, this,
										sourceLink)
							}, "-", {
								label : "OpenInTab",
								command : bindFixed(this.openInTab, this,
			...
			this.StackFrame = domplate(Firebug.Rep, {
				...
				}, "$object|getCallName"), " ( ", FOR("arg",
						"$object|argIterator", TAG("$arg.tag", {
									object : "$arg.value"
								}), SPAN({
									"class" : "arrayComma"
								}, "$arg.delim")), " )", SPAN({
				...
				inspectObject : function(stackFrame, context) {
					var sourceLink = new SourceLink(stackFrame.href,
							stackFrame.lineNo, "js");
					Firebug.chrome.select(sourceLink)
				},
			...
			this.StackTrace = domplate(Firebug.Rep, {
						tag : FOR("frame", "$object.frames focusRow", TAG(
										this.StackFrame.tag, {
											object : "$frame"
										})),
			...
			this.jsdStackFrame = domplate(Firebug.Rep, {
						...
						getTooltip : function(frame, context) {
							...
							var sourceInfo = FBL.getSourceFileAndLineByScript(
									context, frame.script, frame);
							if (sourceInfo) {
								return $STRF("Line", [
												sourceInfo.sourceFile.href,
												sourceInfo.lineNo])
							} else {
								return $STRF("Line", [frame.script.fileName,
												frame.line])
							}
						},
						getContextMenuItems : function(frame, target, context) {
							var fn = frame.script.functionObject
									.getWrappedValue();
							return FirebugReps.Func.getContextMenuItems(fn,
									target, context, frame.script)
						}
					});
			this.ErrorMessage = domplate(Firebug.Rep, {
				...
				getSourceLink : function(error) {
					...
					return error.lineNo ? new SourceLink(error.href,
							error.lineNo, ext) : null
				},
				getSourceType : function(error) {
					if (error.source) {
						...
						if (error.lineNo == 1
								&& getFileExtension(error.href) != "js") {
							return "none"
						} else {
					...
					if (hasClass(event.target, "errorBreak")) {
						...
						if (hasClass(event.target, "errorSource")) {
							...
							if (hasClass(event.target, "errorTitle")) {
								...
								event.target.setAttribute("aria-checked",
										hasClass(target, "opened"));
								if (hasClass(target, "opened")) {
									if (target.stackTrace) {
										var node = FirebugReps.StackTrace.tag
												.append({
									...
									if (Firebug.A11yModel.enabled) {
										var panel = Firebug
												.getElementPanel(event.target);
										dispatch([Firebug.A11yModel],
												"onLogRowContentCreated", [
														panel, traceBox])
									}
								} else {
									clearNode(traceBox)
				...
				copyError : function(error) {
					var message = [this.getMessage(error.message), error.href,
							"Line " + error.lineNo];
					copyToClipboard(message.join("\n"))
				},
				breakOnThisError : function(error) {
					if (this.hasErrorBreak(error)) {
						Firebug.Debugger.clearErrorBreakpoint(error.href,
								error.lineNo)
					} else {
						Firebug.Debugger.setErrorBreakpoint(error.href,
								error.lineNo)
					}
				},
				...
				getContextMenuItems : function(error, target, context) {
					...
					if (error.category == "css") {
						items.push("-", {
									...
									command : bindFixed(this.breakOnThisError,
											this, error)
								}, optionMenu("BreakOnAllErrors",
										"breakOnErrors"))
					}
					return items
				}
			...
			this.Assert = domplate(Firebug.Rep, {
				...
				getContextMenuItems : function(error, target, context) {
					...
					return [{
								...
								command : bindFixed(this.breakOnAllErrors,
										this, error)
							}]
				}
			});
			this.SourceText = domplate(Firebug.Rep, {
						...
						getHTML : function(sourceText) {
							return getSourceLineRange(sourceText, 1,
									sourceText.lines.length)
						}
					});
			...
			this.ApplicationCache = domplate(Firebug.Rep, {
				...
				summarizeCache : function(applicationCache) {
					try {
						return applicationCache.length
								+ " items in offline cache"
					} catch (exc) {
			...
			Firebug.registerRep(this.Undefined, this.Null, this.Number,
					this.String, this.Window, this.Element, this.Document,
					this.StyleSheet, this.Event, this.Property, this.Except,
					this.Arr);
		...
		with (FBL) {
			...
			Firebug.Editor = extend(Firebug.Module, {
				...
				startEditing : function(target, value, editor) {
					this.stopEditing();
					if (hasClass(target, "insertBefore")
				...
				stopEditing : function(cancel) {
					...
					if (FBTrace.DBG_EDITOR) {
						FBTrace.sysout("editor.stopEditing cancel:" + cancel
					...
					var removeGroup = currentEditor.endEditing(currentTarget,
							value, cancel);
					try {
						if (cancel) {
							if (value != originalValue) {
								this.saveEditAndNotifyListeners(currentTarget,
										originalValue, previousValue)
							}
							if (removeGroup && !originalValue && currentGroup) {
								currentGroup.parentNode
										.removeChild(currentGroup)
							}
						} else {
							if (!value) {
								this.saveEditAndNotifyListeners(currentTarget,
				...
				update : function(saveNow) {
					...
					if (saveNow) {
						...
						this.saveTimeout = context.setTimeout(bindFixed(
										this.save, this), saveTimeout);
						if (FBTrace.DBG_EDITOR) {
							FBTrace.sysout("editor.update saveTimeout: "
									+ this.saveTimeout)
						}
					}
				},
				save : function(value) {
					...
					if (FBTrace.DBG_EDITOR) {
						FBTrace.sysout("editor.save saveTimeout: "
								+ this.saveTimeout + " currentPanel: "
								+ (currentPanel ? currentPanel.name : "null"))
					}
					try {
						this.saveEditAndNotifyListeners(currentTarget, value,
								previousValue);
						previousValue = value;
						invalidEditor = false
					} catch (exc) {
				...
				tabPreviousEditor : function() {
					...
					do {
						prevEditable = !value && currentGroup
								? getPreviousOutsider(prevEditable,
										currentGroup)
				...
				insertRow : function(relative, insertWhere) {
					...
					currentGroup = currentEditor.insertNewRow(group,
							insertWhere);
					if (!currentGroup) {
						return
					}
				...
				attachListeners : function(editor, context) {
					...
					var chrome = Firebug.chrome;
					this.listeners = [chrome.keyCodeListen("ESCAPE", null,
							bind(this.cancelEditing, this))];
					if (editor.arrowCompletion) {
						this.listeners.push(chrome.keyCodeListen("UP", null,
										bindFixed(editor.completeValue, editor,
												-1)), chrome.keyCodeListen(
										"DOWN", null,
										bindFixed(editor.completeValue, editor,
												1)), chrome.keyCodeListen(
										"PAGE_UP", null, bindFixed(
												editor.completeValue, editor,
												-pageAmount)), chrome
										.keyCodeListen("PAGE_DOWN", null,
												bindFixed(editor.completeValue,
														editor, pageAmount)))
					}
					if (currentEditor.tabNavigation) {
						this.listeners.push(chrome.keyCodeListen("RETURN",
										null, bind(this.tabNextEditor, this)),
								chrome.keyCodeListen("RETURN", isControl, bind(
												this.insertRow, this, null,
												"after")), chrome
										.keyCodeListen("TAB", null, bind(
														this.tabNextEditor,
														this)), chrome
										.keyCodeListen("TAB", isShift, bind(
														this.tabPreviousEditor,
														this)))
					} else {
						if (currentEditor.multiLine) {
							this.listeners.push(chrome.keyCodeListen("TAB",
									null, insertTab))
						} else {
							this.listeners.push(chrome.keyCodeListen("RETURN",
									null, bindFixed(this.stopEditing, this)));
							if (currentEditor.tabCompletion) {
								this.listeners.push(chrome.keyCodeListen("TAB",
												null, bind(
														editor.completeValue,
														editor, 1)),
										chrome.keyCodeListen("TAB", isShift,
												bind(editor.completeValue,
														editor, -1)))
							}
						}
					}
			...
			Firebug.InlineEditor.prototype = domplate(Firebug.BaseEditor, {
				...
				initializeInline : function(doc) {
					if (FBTrace.DBG_EDITOR) {
						FBTrace
								.sysout("Firebug.InlineEditor initializeInline()")
					}
				...
				show : function(target, panel, value, targetSize) {
					...
					if (this.fixedWidth) {
						...
						if (hasClass(parent, "textEditorInner2")) {
							var yDiff = this.textSize.height
									- this.shadowExpand;
							if (isIE6) {
								yDiff -= 2
							}
					...
					if (!this.fixedWidth) {
						...
						this.expander.parentNode.insertBefore(target,
								this.expander)
					}
				...
				getAutoCompleter : function() {
					if (!this.autoCompleter) {
						this.autoCompleter = new Firebug.AutoCompleter(null,
								bind(this.getAutoCompleteRange, this), bind(
										this.getAutoCompleteList, this), true,
								false)
					}
					return this.autoCompleter
				},
				completeValue : function(amt) {
					if (this.getAutoCompleter().complete(currentPanel.context,
							this.input, true, amt < 0)) {
				...
				incrementValue : function(amt) {
					...
					var expr = value.substr(range.start, range.end
									- range.start + 1);
					...
					if (!!intValue || intValue == 0) {
						...
						this.input.value = preExpr + completion + digitPost
								+ postExpr;
				...
				onKeyPress : function(event) {
					if (event.keyCode == 27 && !this.completeAsYouType) {
						var reverted = this.getAutoCompleter()
								.revert(this.input);
						if (reverted) {
							cancelEvent(event)
						}
						...
						if (event.charCode
								&& this.advanceToNext(this.target,
										event.charCode)) {
				...
				onInput : function() {
					if (this.ignoreNextInput) {
						...
						if (this.completeAsYouType) {
							this.getAutoCompleter().complete(
									currentPanel.context, this.input, false)
						} else {
							this.getAutoCompleter().reset()
						}
					}
				...
				updateLayout : function(initial, forceAll, extraWidth) {
					if (this.fixedWidth) {
						...
						var wrapped = initial
								? this.noWrap
										&& this.targetSize.height > this.textSize.height
												+ 3
						...
						if (wrapped) {
							...
							targetMargin = parseInt(style.marginLeft)
							...
							if (initial) {
								if (isIE) {
									var xDiff = 13;
									this.box.style.width = (inputWidth + xDiff)
											+ "px"
								} else {
									this.box.style.width = "auto"
								}
							} else {
								var xDiff = isIE ? 13 : this.box.scrollWidth
						...
						this.expander.style.height = (this.textSize.height - 3)
								+ "px"
					}
					if (forceAll) {
			...
			Firebug.AutoCompleter = function(getExprOffset, getRange,
					evaluator, selectMode, caseSensitive) {
				var candidates = null;
				var originalValue = null;
				...
				this.revert = function(textBox) {
					if (originalOffset != -1) {
						...
						setSelectionRange(textBox, originalOffset,
								originalOffset);
						this.reset();
						return true
					} else {
				...
				this.complete = function(context, textBox, cycle, reverse) {
					...
					if (!candidates || !cycle || offset != lastOffset) {
						...
						originalValue = value;
						var parseStart = getExprOffset ? getExprOffset(value,
								offset, context) : 0;
						preParsed = value.substr(0, parseStart);
						var parsed = value.substr(parseStart);
						var range = getRange ? getRange(parsed, offset
										- parseStart, context) : null;
						if (!range) {
							range = {
								start : 0,
								end : parsed.length - 1
						...
						var expr = parsed.substr(range.start, range.end
										- range.start + 1);
						...
						if (expr) {
							...
							if (caseSensitive) {
								...
								var lowerExpr = caseSensitive ? expr : expr
										.toLowerCase();
								for (var i = 0; i < values.length; ++i) {
									var name = values[i];
									if (name.indexOf
					...
					textBox.value = preParsed + preExpr + preCompletion
							+ postCompletion + postExpr;
					var offsetEnd = preParsed.length + preExpr.length
							+ completion.length;
					setTimeout(function() {
								if (selectMode) {
									setSelectionRange(textBox, offset,
											offsetEnd)
								} else {
									setSelectionRange(textBox, offsetEnd,
											offsetEnd)
								}
							}, 0);
					return true
			...
			var getOutsider = function getOutsider(element, group, stepper) {
				var parentGroup = getAncestorByClass(group.parentNode,
						"editGroup");
				var next;
				do {
					next = stepper(next || element)
				} while (isAncestor(next, group)
						|| isGroupInsert(next, parentGroup));
				return next
			};
			var isGroupInsert = function isGroupInsert(next, group) {
				return (!group || isAncestor(next, group))
						&& (hasClass(next, "insertBefore") || hasClass(next,
								"insertAfter"))
			};
			var getNextOutsider = function getNextOutsider(element, group) {
				return getOutsider(element, group, bind(getNextByClass, FBL,
								"editable"))
			};
			var getPreviousOutsider = function getPreviousOutsider(element,
					group) {
				return getOutsider(element, group, bind(getPreviousByClass,
								FBL, "editable"))
			};
			...
			var insertTab = function insertTab() {
				insertTextIntoElement(currentEditor.input,
						Firebug.Editor.tabCharacter)
			};
	...
	FBL.ns(function() {
		with (FBL) {
			...
			Firebug.Inspector = {
				create : function() {
					offlineFragment = Env.browser.document
				...
				startInspecting : function() {
					...
					addEvent(fbInspectFrame, "mousemove",
							Firebug.Inspector.onInspecting);
					addEvent(fbInspectFrame, "mousedown",
				...
				stopInspecting : function() {
					...
					removeEvent(fbInspectFrame, "mousemove",
							Firebug.Inspector.onInspecting);
					removeEvent(fbInspectFrame, "mousedown",
							Firebug.Inspector.onInspectingClick);
				...
				onInspectingClick : function(e) {
					...
					var targ = Firebug.browser.getElementFromPoint(e.clientX,
							e.clientY);
				...
				drawOutline : function(el) {
					...
					var windowSize = Firebug.browser.getWindowSize();
					var scrollSize = Firebug.browser.getWindowScrollSize();
					var scrollPosition = Firebug.browser
					...
					var width = box.width;
					var freeHorizontalSpace = scrollPosition.left
							+ windowSize.width
							- left
							- width
							- (!isIE && scrollSize.height > windowSize.height
									? scrollbarSize
									: 0);
					var freeVerticalSpace = scrollPosition.top
							+ windowSize.height
							- top
							- height
							- (!isIE && scrollSize.width > windowSize.width
									? scrollbarSize
									: 0);
					...
					if (freeHorizontalSpace > 0) {
						...
						style.height = height + numVerticalBorders * border
								+ "px";
						style.width = (freeHorizontalSpace < border
								? freeHorizontalSpace
								: border)
								+ "px"
					} else {
				...
				showOutline : function() {
					...
					for (var name in outline) {
						Firebug.browser.document.getElementsByTagName("body")[0]
				...
				drawBoxModel : function(el) {
					...
					var scrollPosition = Firebug.browser
							.getWindowScrollPosition();
					var offsetHeight = Firebug.chrome.type == "frame"
							? FirebugChrome.height
							: 0;
					if (box.top > scrollPosition.top + windowSize.height
							- offsetHeight
							|| box.left > scrollPosition.left
									+ windowSize.width
					...
					var height = box.height;
					var width = box.width;
					var margin = Firebug.browser
							.getMeasurementBox(el, "margin");
					var padding = Firebug.browser.getMeasurementBox(el,
							"padding");
					var border = Firebug.browser
							.getMeasurementBox(el, "border");
					...
					boxPaddingStyle.height = height - border.top
							- border.bottom + "px";
					boxPaddingStyle.width = width - border.left - border.right
							+ "px";
					boxContentStyle.top = margin.top + border.top + padding.top
							+ "px";
					boxContentStyle.left = margin.left + border.left
							+ padding.left + "px";
					boxContentStyle.height = height - border.top - padding.top
							- padding.bottom - border.bottom + "px";
					boxContentStyle.width = width - border.left - padding.left
							- padding.right - border.right + "px";
					if (!boxModelVisible) {
						this.showBoxModel()
					}
				},
			...
			var inspectFrameStyle = resetStyle
			...
			var inspectMarginStyle = inspectStyle
					+ "background: #EDFF64; height:100%; width:100%;";
			...
			var createOutlineInspector = function createOutlineInspector() {
				for (var name in outline) {
					...
					el.style.cssText = inspectStyle
							+ outlineStyle[outline[name]];
	...
	FBL.ns(function() {
		with (FBL) {
			...
			Firebug.ConsoleBase = {
				log : function(object, context, className, rep, noThrottle,
						sourceLink) {
					return this.logRow(appendObject, object, context,
							className, rep, sourceLink, noThrottle)
				},
				logFormatted : function(objects, context, className,
						noThrottle, sourceLink) {
					return this.logRow(appendFormatted, objects, context,
							className, null, sourceLink, noThrottle)
				},
				openGroup : function(objects, context, className, rep,
						noThrottle, sourceLink, noPush) {
					return this.logRow(appendOpenGroup, objects, context,
							className, rep, sourceLink, noThrottle)
				},
				closeGroup : function(context, noThrottle) {
					return this.logRow(appendCloseGroup, null, context, null,
							null, null, noThrottle, true)
				},
				logRow : function(appender, objects, context, className, rep,
						sourceLink, noThrottle, noRow) {
					noThrottle = true;
					if (!context) {
					...
					if (noThrottle || !context) {
						...
						if (panel) {
							var row = panel.append(appender, objects,
									className, rep, sourceLink, noRow);
							var container = panel.panelNode;
							return row
						} else {
							consoleQueue.push([appender, objects, context,
									className, rep, sourceLink, noThrottle,
									noRow])
						}
					} else {
						...
						var args = [appender, objects, context, className, rep,
								sourceLink, true, noRow];
				...
				getPanel : function(context, noCreate) {
					return Firebug.chrome
							? Firebug.chrome.getPanel("Console")
							: null
				}
			};
			...
			Firebug.Console = Firebug.Console = extend(ActivableConsole, {
				...
				error : function() {
					Firebug.Console.logFormatted(arguments, Firebug.browser,
							"error")
				},
				flush : function() {
				...
				getFirebugConsoleElement : function(context, win) {
					var element = win.document
							.getElementById("_firebugConsole");
					if (!element) {
						if (FBTrace.DBG_CONSOLE) {
							FBTrace
									.sysout("getFirebugConsoleElement forcing element")
						}
						...
						if (context.stopped) {
							Firebug.Console.injector
									.evaluateConsoleScript(context)
						} else {
							var r = Firebug.CommandLine.evaluateInWebPage(
									elementForcer, context, win)
						}
						if (FBTrace.DBG_CONSOLE) {
							FBTrace.sysout(
									"getFirebugConsoleElement forcing element result "
											+ r, r)
						}
						var element = win.document
								.getElementById("_firebugConsole");
				...
				isReadyElsePreparing : function(context, win) {
					...
					if (win) {
						...
						for (var i = 0; i < context.windows.length; i++) {
							attached = attached
									&& this.injector.attachIfNeeded(context,
											context.windows[i])
						}
						if (context.windows.indexOf(context.window) == -1) {
							FBTrace
									.sysout("isReadyElsePreparing ***************** context.window not in context.windows")
						}
						if (FBTrace.DBG_CONSOLE) {
							FBTrace
									.sysout("console.isReadyElsePreparing attached to "
				...
				togglePersist : function(context) {
					...
					Firebug.chrome.setGlobalAttribute(
							"cmd_togglePersistConsole", "checked",
							panel.persistContent)
				},
				showContext : function(browser, context) {
					Firebug.chrome.setGlobalAttribute("cmd_clearConsole",
							"disabled", !context);
					Firebug.ActivableModule.showContext.apply(this, arguments)
				},
				destroyContext : function(context, persistedState) {
					Firebug.Console.injector.detachConsole(context,
							context.window)
				},
				...
				onFunctionCall : function(context, frame, depth, calling) {
					if (calling) {
						Firebug.Console.openGroup([frame, "depth:" + depth],
								context)
					} else {
				...
				logRow : function(appender, objects, context, className, rep,
					...
					if (this.isAlwaysEnabled()) {
						return Firebug.ConsoleBase.logRow
			...
			Firebug.ConsolePanel.prototype = extend(Firebug.Panel, {
				...
				groups : null,
				limit : null,
				append : function(appender, objects, className, rep,
						sourceLink, noRow) {
				...
				insertReloadWarning : function() {
					this.warningRow = this.append(appendObject,
					...
					for (var i = 0; i < parts.length; ++i) {
						...
						if (part && typeof(part) == "object") {
							...
							if (typeof(object) != "undefined") {
								...
								this.appendObject(part.type, row,
										FirebugReps.Text)
							}
						} else {
				...
				onMouseMove : function(event) {
					...
					var object = getAncestorByClass(target,
							"objectLink-element");
					object = object ? object.repObject : null;
					if (object && instanceOf(object, "Element")
							&& object.nodeType == 1) {
				...
				onMouseDown : function(event) {
					...
					if (hasClass(object, "objectLink-object")) {
						...
						if (hasClass(object, "objectLink-element")) {
							Firebug.chrome.selectPanel("HTML");
							Firebug.chrome.getPanel("HTML").select(repObject,
									true)
						}
					}
				},
				...
				destroyNode : function() {
					...
					removeEvent(this.resizeEventTarget, "resize",
							this.onResizer)
				},
				shutdown : function() {
				...
				ishow : function(state) {
					...
					if (enabled) {
						...
						if (FBTrace.DBG_CONSOLE) {
							FBTrace
									.sysout("console.show ------------------ wasScrolledToBottom: "
				...
				ihide : function(state) {
					...
					if (FBTrace.DBG_CONSOLE) {
						FBTrace
								.sysout("console.hide ------------------ wasScrolledToBottom: "
										+ this.wasScrolledToBottom
										+ ", "
										+ this.context.getName())
					}
				...
				destroy : function(state) {
					...
					if (FBTrace.DBG_CONSOLE) {
						FBTrace
								.sysout("console.destroy ------------------ wasScrolledToBottom: "
										+ this.wasScrolledToBottom
										+ ", "
										+ this.context.getName())
					}
				...
				shouldBreakOnNext : function() {
					return Firebug.getPref(Firebug.servicePrefDomain,
							"breakOnErrors")
				},
				...
				enablePanel : function(module) {
					if (FBTrace.DBG_CONSOLE) {
						FBTrace.sysout("console.ConsolePanel.enablePanel; "
								+ this.context.getName())
					}
				...
				disablePanel : function(module) {
					if (FBTrace.DBG_CONSOLE) {
						FBTrace.sysout("console.ConsolePanel.disablePanel; "
								+ this.context.getName())
					}
				...
				getOptionsMenuItems : function() {
					return [
							...
							optionMenu("ShowJavaScriptWarnings",
									"showJSWarnings"),
							optionMenu("ShowCSSErrors", "showCSSErrors"),
							optionMenu("ShowXMLErrors", "showXMLErrors"),
							optionMenu("ShowXMLHttpRequests",
									"showXMLHttpRequests"),
							optionMenu("ShowChromeErrors", "showChromeErrors"),
							optionMenu("ShowChromeMessages",
									"showChromeMessages"),
							optionMenu("ShowExternalErrors",
									"showExternalErrors"),
				...
				getShowStackTraceMenuItem : function() {
					var menuItem = serviceOptionMenu("ShowStackTrace",
							"showStackTrace");
				...
				getStrictOptionMenuItem : function() {
					...
					var strictValue = prefs.getBoolPref(strictDomain + "."
							+ strictName);
					return {
				...
				search : function(text) {
					...
					if (!logRow) {
						dispatch([Firebug.A11yModel],
								"onConsoleSearchMatchFound", [this, text, []]);
						return false
					}
				...
				breakOnNext : function(breaking) {
					Firebug.setPref(Firebug.servicePrefDomain, "breakOnErrors",
							breaking)
				},
				createRow : function(rowName, className) {
					var elt = this.document.createElement("div");
					elt.className = rowName
							+ (className ? " " + rowName + "-" + className : "");
					return elt
				},
				...
				showCommandLine : function(shouldShow) {
					...
					if (shouldShow) {
						...
						Firebug.CommandLine.setMultiLine(
								Firebug.largeCommandLine, Firebug.chrome)
					} else {
				...
				onScroll : function(event) {
					this.wasScrolledToBottom = FBL
							.isScrolledToBottom(this.panelNode);
					if (FBTrace.DBG_CONSOLE) {
						FBTrace.sysout(
		...
		with (FBL) {
			...
			Firebug.Console.injector = {
				install : function(context) {
					...
					var properties = ["log", "debug", "info", "warn", "error",
							"assert", "dir", "dirxml", "group",
							"groupCollapsed", "groupEnd", "time", "timeEnd",
							"count", "trace", "profile", "profileEnd", "clear",
							"open", "close"];
					...
					var consoleNS = (!isFirefox || isFirefox
							&& !("console" in win)) ? "console" : "firebug";
					var sandbox = new win.Function("arguments.callee.install(window."
							+ consoleNS + "={})");
				...
				isAttached : function(context, win) {
					if (win.wrappedJSObject) {
						...
						if (FBTrace.DBG_CONSOLE) {
							FBTrace.sysout("Console.isAttached? to win "
									+ win.location + " fnc:"
				...
				attachIfNeeded : function(context, win) {
					if (FBTrace.DBG_CONSOLE) {
						FBTrace
								.sysout("Console.attachIfNeeded has win "
										+ (win
												? ((win.wrappedJSObject
														? "YES"
														: "NO") + " wrappedJSObject")
					...
					if (FBTrace.DBG_CONSOLE) {
						FBTrace
								.sysout("Console.attachIfNeeded found isAttached false ")
					}
					...
					if (attached) {
						dispatch(Firebug.Console.fbListeners,
								"onConsoleInjected", [context, win])
					}
					return attached
				},
				attachConsoleInjector : function(context, win) {
					...
					if (FBTrace.DBG_CONSOLE) {
						FBTrace.sysout("attachConsoleInjector evaluating in "
										+ win.location, consoleInjection)
					}
					Firebug.CommandLine.evaluateInWebPage(consoleInjection,
							context, win);
					if (FBTrace.DBG_CONSOLE) {
						FBTrace
								.sysout("attachConsoleInjector evaluation completed for "
										+ win.location)
					}
				},
				...
				forceConsoleCompilationInPage : function(context, win) {
					if (!win) {
						if (FBTrace.DBG_CONSOLE) {
							FBTrace
									.sysout("no win in forceConsoleCompilationInPage!")
						}
						return
					}
					...
					if (context.stopped) {
						...
						Firebug.CommandLine.evaluateInWebPage(consoleForcer,
								context, win)
					}
					if (FBTrace.DBG_CONSOLE) {
						FBTrace.sysout("forceConsoleCompilationInPage "
										+ win.location, consoleForcer)
					}
				},
				...
				addConsoleListener : function(context, win) {
					if (!context.activeConsoleHandlers) {
						...
						for (var i = 0; i < context.activeConsoleHandlers.length; i++) {
							if (context.activeConsoleHandlers[i].window == win) {
								...
								if (FBTrace.DBG_CONSOLE) {
									FBTrace
											.sysout("consoleInjector addConsoleListener removed handler("
					...
					if (FBTrace.DBG_CONSOLE) {
						FBTrace
								.sysout("consoleInjector addConsoleListener attached handler("
			...
			var FirebugConsoleHandler = function FirebugConsoleHandler(context,
					win) {
				this.window = win;
				this.attachTo = function(element) {
					this.element = element;
					this.boundHandler = bind(this.handleEvent, this);
					this.element.addEventListener("firebugAppendConsole",
							this.boundHandler, true)
				};
				this.detach = function() {
					this.element.removeEventListener("firebugAppendConsole",
							this.boundHandler, true)
				};
				this.handler_name = ++total_handlers;
				this.handleEvent = function(event) {
					if (FBTrace.DBG_CONSOLE) {
						FBTrace.sysout("FirebugConsoleHandler("
										+ this.handler_name
										+ ") "
										+ event.target
					...
					if (!Firebug.CommandLine.CommandHandler.handle(event, this,
							win)) {
						if (FBTrace.DBG_CONSOLE) {
							FBTrace.sysout("FirebugConsoleHandler", this)
						}
						var methodName = event.target
								.getAttribute("methodName");
						Firebug.Console.log($STRF("console.MethodNotSupported",
								[methodName]))
					}
				};
				this.firebuglite = Firebug.version;
				this.init = function() {
					var consoleElement = win.document
							.getElementById("_firebugConsole");
					consoleElement.setAttribute("FirebugVersion",
							Firebug.version)
				};
				this.log = function() {
				...
				this.dir = function(o) {
					Firebug.Console.log(o, context, "dir",
				...
				this.dirxml = function(o) {
					...
					Firebug.Console.log(o, context, "dirxml",
							Firebug.HTMLPanel.SoloElement)
				};
				this.group = function() {
					var sourceLink = null;
					Firebug.Console.openGroup(arguments, null, "group", null,
							false, sourceLink)
				};
				...
				this.groupCollapsed = function() {
					...
					var row = Firebug.Console.openGroup(arguments, null,
							"group", null, true, sourceLink);
					removeClass(row, "opened")
				};
				this.profile = function(title) {
					logFormatted(["console.profile() not supported."], "warn",
							true)
				};
				...
				this.count = function(key) {
					...
					if (frameId) {
						...
						var label = key == undefined ? frameCounter.count : key
								+ " " + frameCounter.count;
						...
						if (FBL.isSafari) {
							...
							for (var i = 4, length = items.length; i < length; i++, framePos++) {
								...
								if (match) {
									...
									if (name) {
										name = name.replace(
												reChromeStackItemName, "");
										frame.name = name
									}
									var value = match[2]
											.match(reChromeStackItemValue);
									if (value) {
										frame.href = value[1];
				...
				function logFormatted(args, className, linkToSource, noThrottle) {
					...
					return Firebug.Console.logFormatted(args, context,
							className, noThrottle, sourceLink)
				}
				function logAssert(category, args) {
					...
					if (Firebug.errorStackTrace) {
						...
						if (FBTrace.DBG_CONSOLE) {
							FBTrace.sysout(
									"logAssert trace from errorStackTrace",
									trace)
						}
					} else {
						if (msg.stack) {
							...
							if (FBTrace.DBG_CONSOLE) {
								FBTrace
										.sysout(
												"logAssert trace from msg.stack",
												trace)
							}
						} else {
							...
							if (FBTrace.DBG_CONSOLE) {
								FBTrace.sysout(
										"logAssert trace from getJSDUserStack",
										trace)
							}
						}
					}
					var errorObject = new FBL.ErrorMessage(msg, (msg.fileName
									? msg.fileName
									: win.location), (msg.lineNumber
									? msg.lineNumber
									: 0), "", category, context, trace);
					...
					var row = Firebug.Console.log(objects, context,
				...
				function getJSDUserStack() {
					...
					if (frames && (frames.length > 0)) {
						...
						FBTrace.sysout("consoleInjector getJSDUserStack: "
										+ frames.length + " oldest: " + oldest
										+ " i: " + i + " i - oldest + 2: "
										+ (i - oldest + 2), trace);
		...
		with (FBL) {
			...
			Firebug.CommandLine = extend(Firebug.Module, {
				...
				initialize : function(doc) {
					...
					this.onMultiLineKeyDown = bind(this.onMultiLineKeyDown,
							this);
				...
				activate : function(multiLine, hideToggleIcon, onRun) {
					...
					if (multiLine) {
						...
						addEvent(this.element, "keydown",
								this.onMultiLineKeyDown);
				...
				evaluate : function(expr) {
					...
					var result = Firebug.context.evaluate(expr, "window", api,
				...
				enter : function() {
					...
					_stack(command);
					Firebug.Console.log(commandPrefix + " "
									+ stripNewLines(command), Firebug.browser,
							"command", FirebugReps.Text);
				...
				autocomplete : function(reverse) {
					...
					if (!isAutoCompleting) {
						...
						if (obj) {
							...
							autoCompleteExpr = valBegin + commandBegin
									+ (objName ? objName + "." : "");
					...
					if (buffer) {
						...
						for (var i = autoCompletePosition + diff, l = buffer.length, bi; i >= 0
								&& i < l; i += diff) {
							bi = buffer[i];
				...
				onError : function(msg, href, lineNo) {
					...
					var fileName = lastSlash == -1 ? href : href
							.substr(lastSlash + 1);
					html.push('<span class="errorMessage">', msg, "</span>",
			...
			var CommandLineAPI = {
				...
				$$ : function(selector, context) {
					...
					return Firebug.Selector ? Firebug.Selector(selector,
							context) : Firebug.Console
							.error("Firebug.Selector module not loaded.")
				},
				$0 : null,
				$1 : null,
				dir : function(o) {
					Firebug.Console.log(o, Firebug.context, "dir",
							Firebug.DOMPanel.DirTable)
				},
				dirxml : function(o) {
					if (o instanceof Window) {
					...
					Firebug.Console.log(o, Firebug.context, "dirxml",
							Firebug.HTMLPanel.SoloElement)
				}
			};
		...
		with (FBL) {
			...
			var XMLHttpRequestWrapper = function(activeXObject) {
				var xhrRequest = typeof activeXObject != "undefined"
						? activeXObject
						: new _XMLHttpRequest(), spy = new XHRSpy(), self = this, reqType, reqUrl, reqStartTS;
				var logXHR = function() {
					var row = Firebug.Console.log(spy, null, "spy",
							Firebug.Spy.XHR);
					if (row) {
				...
				var finishXHR = function() {
					...
					var responseHeadersText = xhrRequest
							.getAllResponseHeaders();
					var responses = responseHeadersText ? responseHeadersText
							.split(/[\n\r]/) : [];
					var reHeader = /^(\S+):\s*(.*)/;
		...
		with (FBL) {
			...
			Firebug.NetMonitor = extend(Firebug.ActivableModule, {
						...
						initialize : function() {
							...
							if (Firebug.TraceModule) {
								Firebug.TraceModule
										.addListener(this.TraceListener)
							}
						...
						shutdown : function() {
							return;
							prefs.removeObserver(Firebug.prefDomain, this,
									false);
							if (Firebug.TraceModule) {
								Firebug.TraceModule
										.removeListener(this.TraceListener)
							}
							NetHttpObserver.unregisterObserver();
							NetHttpActivityObserver.unregisterObserver();
							Firebug.Debugger
									.removeListener(this.DebuggerListener)
						}
					});
			Firebug.NetMonitor.NetInfoBody = domplate(Firebug.Rep, {
				...
				appendTab : function(netInfoBox, tabId, tabTitle) {
					...
					this.customTab.append(args, netInfoBox
									.getElementsByClassName("netInfoTabs")
									.item(0));
					this.customBody.append(args, netInfoBox
									.getElementsByClassName("netInfoBodies")
									.item(0))
				},
				selectTabByName : function(netInfoBox, tabName) {
					var tab = getChildByClass(netInfoBox, "netInfoTabs",
							"netInfo" + tabName + "Tab");
					if (tab) {
						this.selectTab(tab)
					}
				...
				selectTab : function(tab) {
					...
					if (selectedTab) {
						removeClass(netInfoBox.selectedText,
								"netInfoTextSelected");
				...
				updateInfo : function(netInfoBox, file, context) {
					...
					if (!netInfoBox) {
						if (FBTrace.DBG_NET || FBTrace.DBG_ERRORS) {
							FBTrace.sysout(
									"net.updateInfo; ERROR netInfo == null "
											+ file.href, file)
						}
						return
					}
						...
						if (hasClass(tab, "netInfoHeadersTab")) {
							...
							if (hasClass(tab, "netInfoPostTab")) {
								if (!netInfoBox.postPresented) {
									...
									var postText = $$(".netInfoPostText",
											netInfoBox)[0];
									NetInfoPostData.render(context, postText,
											file)
								}
							} else {
								if (hasClass(tab, "netInfoPutTab")) {
									if (!netInfoBox.putPresented) {
										...
										var putText = $$(".netInfoPutText",
												netInfoBox)[0];
										NetInfoPostData.render(context,
												putText, file)
									}
								} else {
									if (hasClass(tab, "netInfoResponseTab")
											&& file.loaded
										...
										if (file.category == "image") {
											...
											var responseImage = netInfoBox.ownerDocument
													.createElement("img");
											responseImage.src = file.href;
											clearNode(responseTextBox);
											responseTextBox.appendChild(
													responseImage,
													responseTextBox)
										} else {
											this.setResponseText(file,
													netInfoBox,
													responseTextBox, context)
										}
									} else {
										if (hasClass(tab, "netInfoCacheTab")
												&& file.loaded
											...
											if (file.cacheEntry) {
												...
												this.insertHeaderRows(
														netInfoBox,
														file.cacheEntry,
														"Cache")
											}
										} else {
											if (hasClass(tab, "netInfoHtmlTab")
													&& file.loaded
													&& !netInfoBox.htmlPresented) {
												netInfoBox.htmlPresented = true;
												var text = Utils
														.getResponseText(file,
																context);
												var iframe = netInfoBox
														.getElementsByClassName("netInfoHtmlPreview")
														.item(0);
				...
				setResponseText : function(file, netInfoBox, responseTextBox,
					...
					responseTextBox[typeof responseTextBox.textContent != "undefined"
							? "textContent"
							: "innerText"] = file.responseText;
					return;
					...
					if (limitReached) {
						...
						Firebug.NetMonitor.ResponseSizeLimit.append(object,
								responseTextBox)
					}
					netInfoBox.responsePresented = true;
					if (FBTrace.DBG_NET) {
						FBTrace
								.sysout("net.setResponseText; response text updated")
					}
				},
				insertHeaderRows : function(netInfoBox, headers, tableName,
						rowName) {
					if (!headers.length) {
						return
					}
					var headersTable = $$(".netInfo" + tableName + "Table",
							netInfoBox)[0];
					var tbody = getChildByClass(headersTable, "netInfo"
									+ rowName + "Body");
					if (!tbody) {
						tbody = headersTable.firstChild
					}
					var titleRow = getChildByClass(tbody, "netInfo" + rowName
									+ "Title");
			...
			Firebug.NetMonitor.NetInfoHeaders = domplate(Firebug.Rep, {
				...
				onViewSource : function(event) {
					...
					if (target.sourceDisplayed) {
						var headers = requestHeaders
								? file.requestHeaders
								: file.responseHeaders;
						this.insertHeaderRows(netInfoBox, headers,
								target.rowName);
				...
				insertSource : function(netInfoBox, source, rowName) {
					var tbody = netInfoBox.getElementsByClassName("netInfo"
							+ rowName + "Body").item(0);
					var node = this.sourceTag.replace({}, tbody);
					var sourceNode = node.getElementsByClassName("source")
							.item(0);
				...
				insertHeaderRows : function(netInfoBox, headers, rowName) {
					...
					var titleRow = getChildByClass(headersTable, "netInfo"
									+ rowName + "Title");
					removeClass(titleRow, "collapsed")
				},
			...
			Firebug.NetMonitor.NetInfoPostData = domplate(Firebug.Rep, {
				...
				insertJSON : function(parentNode, file, context) {
					...
					var jsonBody = jsonTable
							.getElementsByClassName("netInfoPostJSONBody")
							.item(0);
					if (!this.toggles) {
						this.toggles = {}
					}
				...
				insertXML : function(parentNode, file, context) {
					...
					var jsonBody = jsonTable
							.getElementsByClassName("netInfoPostXMLBody")
							.item(0);
			...
			Firebug.NetMonitor.NetLimit = domplate(Firebug.Rep, {
				...
				updateCounter : function(row) {
					...
					var limitLabel = row
							.getElementsByClassName("netLimitLabel").item(0);
					limitLabel.firstChild.nodeValue = $STRP(
							"plural.Limit_Exceeded", [row.limitInfo.totalCount])
				},
				...
				updateMaxLimit : function() {
					var value = Firebug.getPref(Firebug.prefDomain,
							"net.logLimit");
					maxQueueRequests = value ? value : maxQueueRequests
				}
			});
			...
			Firebug.NetMonitor.Utils = {
				...
				getPostText : function(file, context, noLimit) {
					if (!file.postText) {
						file.postText = readPostTextFromRequest(file.request,
								context);
						if (!file.postText && context) {
							file.postText = readPostTextFromPage(file.href,
									context)
						}
					}
					if (!file.postText) {
				...
				isURLEncodedRequest : function(file, context) {
					...
					var headerValue = Utils.findHeader(file.requestHeaders,
							"content-type");
					if (headerValue
							&& headerValue
				...
				isMultiPartRequest : function(file, context) {
					...
					if (text
							&& text
									.toLowerCase()
									.indexOf("content-type: multipart/form-data") == 0) {
						return true
					}
					return false
				},
				getMimeType : function(mimeType, uri) {
					if (!mimeType
							|| !(mimeCategoryMap.hasOwnProperty(mimeType))) {
						var ext = getFileExtension(uri);
						if (!ext) {
							return mimeType
						} else {
							var extMimeType = mimeExtensionMap[ext
									.toLowerCase()];
				...
				getHttpHeaders : function(request, file) {
					try {
						...
						if (FBTrace.DBG_ERRORS) {
							FBTrace.sysout("net.getHttpHeaders FAILS "
											+ file.href, exc)
						}
					}
				},
				isXHR : function(request) {
					try {
						...
						var xhrRequest = callbacks ? callbacks
								.getInterface(Ci.nsIXMLHttpRequest) : null;
						if (FBTrace.DBG_NET) {
				...
				getFileCategory : function(file) {
					if (file.category) {
						if (FBTrace.DBG_NET) {
							FBTrace.sysout("net.getFileCategory; current: "
											+ file.category + " for: "
											+ file.href, file)
						}
					...
					if (file.isXHR) {
						if (FBTrace.DBG_NET) {
							FBTrace.sysout("net.getFileCategory; XHR for: "
											+ file.href, file)
						}
		...
		with (FBL) {
			...
			Firebug.Spy = extend(Firebug.Module, {
				...
				initContext : function(context) {
					...
					if (FBTrace.DBG_SPY) {
						FBTrace.sysout("spy.initContext " + contexts.length
										+ " ", context.getName())
					}
				},
				destroyContext : function(context) {
					...
					if (FBTrace.DBG_SPY && context.spies.length) {
						FBTrace
								.sysout("spy.destroyContext; ERROR There are leaking Spies ("
					...
					if (FBTrace.DBG_SPY) {
						FBTrace.sysout("spy.destroyContext " + contexts.length
										+ " ", context.getName())
					}
				},
				...
				attachObserver : function(context, win) {
					...
					if (contexts.length == 0) {
						httpObserver.addObserver(SpyHttpObserver,
								"firebug-http-event", false);
					...
					if (FBTrace.DBG_SPY) {
						FBTrace.sysout("spy.attachObserver (HTTP) "
				...
				detachObserver : function(context, win) {
					for (var i = 0; i < contexts.length; ++i) {
						if (contexts[i].context == context) {
							...
							contexts.splice(i, 1);
							if (contexts.length == 0) {
								httpObserver.removeObserver(SpyHttpObserver,
										"firebug-http-event");
								SpyHttpActivityObserver.unregisterObserver()
							}
							if (FBTrace.DBG_SPY) {
								FBTrace.sysout("spy.detachObserver (HTTP) "
												+ contexts.length + " ",
										context.getName())
							}
							return
				...
				getXHR : function(request) {
					...
					try {
						...
						if (exc.name == "NS_NOINTERFACE") {
							if (FBTrace.DBG_SPY) {
								FBTrace
										.sysout("spy.getXHR; Request is not nsIXMLHttpRequest: "
			...
			Firebug.Spy.XHR = domplate(Firebug.Rep, {
									...
									"aria-expanded" : "false"
											...
											role : "presentation"
												...
												TD(
														...
														TAG(
																FirebugReps.SourceLink.tag,
																{
																	object : "$object.sourceLink"
																})))))),
				getCaption : function(spy) {
					return spy.method.toUpperCase() + " "
							+ cropString(spy.getURL(), 100)
				},
				...
				onToggleBody : function(event) {
					...
					if (isLeftClick(event)) {
						...
						var spyHeadTable = getAncestorByClass(target,
								"spyHeadTable");
						if (hasClass(logRow, "opened")) {
							updateHttpSpyInfo(spy, logRow);
							if (spyHeadTable) {
								spyHeadTable.setAttribute("aria-expanded",
										"true")
							}
						} else {
						}
				...
				getContextMenuItems : function(spy) {
					...
					if (spy.postText) {
						items.push({
									...
									command : bindFixed(this.copyParams, this,
											spy)
								})
					}
					items.push({
								label : "CopyResponse",
								command : bindFixed(this.copyResponse, this,
										spy)
							}, "-", {
								label : "OpenInTab",
			...
			function updateTime(spy) {
				var timeBox = spy.logRow.getElementsByClassName("spyTime")
						.item(0);
				if (spy.responseTime) {
			...
			function updateLogRow(spy) {
				updateTime(spy);
				var statusBox = spy.logRow.getElementsByClassName("spyStatus")
						.item(0);
		...
		with (FBL) {
			...
			Firebug.HTML = extend(Firebug.Module, {
				appendTreeNode : function(nodeArray, html) {
					...
					for (var n = 0, node; node = nodeArray[n]; n++) {
						if (node.nodeType == 1) {
							...
							var hasSingleTextChild = childLength == 1
									&& node.firstChild.nodeType == 3
									&& nodeName != "script"
									&& nodeName != "style";
							var nodeControl = !hasSingleTextChild
									&& childLength > 0
							...
							if (typeof uid != "undefined") {
								html.push('<div class="objectBox-element" ',
										'id="', uid, '">', !isIE && nodeControl
												? nodeControl
												: "", "<span ", cacheID, '="',
										uid, '"  class="nodeBox', nodeVisible
												? ""
												: " nodeHidden",
										'">&lt;<span class="nodeTag">',
										nodeName, "</span>")
							...
							if (node.nodeType == 3) {
								if (node.parentNode
										&& (node.parentNode.nodeName
												.toLowerCase() == "script" || node.parentNode.nodeName
												.toLowerCase() == "style")) {
				...
				selectTreeNode : function(id) {
					...
					while (stack.length > 0) {
						...
						node = $(id);
						if (stack.length > 0
								&& documentCache[id].childNodes.length > 0) {
							this.appendTreeChildren(node)
						}
					}
					selectElement(node);
					fbPanel1.scrollTop = Math.round(node.offsetTop
							- fbPanel1.clientHeight / 2)
				}
			});
			...
			HTMLPanel.prototype = extend(Firebug.Panel, {
				...
				initialize : function() {
					...
					if (!selectedElement) {
						Firebug.HTML
								.selectTreeNode(Firebug.browser.document.body[cacheID])
					}
					addEvent(fbPanel1, "mousemove",
							Firebug.HTML.onListMouseMove);
					addEvent($("fbContent"), "mouseout",
							Firebug.HTML.onListMouseMove);
					addEvent(Firebug.chrome.node, "mouseout",
							Firebug.HTML.onListMouseMove)
				},
				shutdown : function() {
					removeEvent(fbPanel1, "mousemove",
							Firebug.HTML.onListMouseMove);
					removeEvent($("fbContent"), "mouseout",
							Firebug.HTML.onListMouseMove);
					removeEvent(Firebug.chrome.node, "mouseout",
							Firebug.HTML.onListMouseMove);
					removeEvent(this.panelNode, "click",
							Firebug.HTML.onTreeClick);
					fbPanel1 = null;
					Firebug.Panel.shutdown.apply(this, arguments)
				},
				reattach : function() {
					if (FirebugChrome.selectedHTMLElementId) {
						Firebug.HTML
								.selectTreeNode(FirebugChrome.selectedHTMLElementId)
					}
				},
				updateSelection : function(object) {
			...
			Firebug.HTML.onTreeClick = function(e) {
				...
				if (targ.className.indexOf("nodeControl") != -1
			...
			Firebug.HTML.onListMouseMove = function onListMouseMove(e) {
				try {
					...
					var found = false;
					while (targ && !found) {
						if (!/\snodeBox\s|\sobjectBox-selector\s/.test(" "
								+ targ.className + " ")) {
							targ = targ.parentNode
						} else {
							found = true
					...
					if (FBL.isIE
							&& " meta title script link ".indexOf(" "
									+ nodeName + " ") != -1) {
						return
					}
					if (!/\snodeBox\s|\sobjectBox-selector\s/.test(" "
					...
					if (el.id == "FirebugUI"
							|| " html head body br script link iframe "
									.indexOf(" " + nodeName + " ") != -1) {
						FBL.Firebug.Inspector.hideBoxModel();
						hoverElement = null;
						return
					}
					if ((new Date().getTime() - hoverElementTS > 40)
							&& hoverElement != el) {
	...
	FBL.ns(function() {
		with (FBL) {
			...
			FBL.processAllStyleSheets = function(doc, styleSheetIterator) {
				...
				if (FBTrace.DBG_CSS) {
					FBTrace.sysout("FBL.processAllStyleSheets",
			...
			var styleGroups = {
				text : ["font-family", "font-size", "font-weight",
						"font-style", "color", "text-transform",
				...
				box : ["width", "height", "top", "right", "bottom", "left",
						...
						"border-left-style", "-moz-border-top-radius",
						"-moz-border-right-radius",
						"-moz-border-bottom-radius", "-moz-border-left-radius",
						"outline-top-width", "outline-right-width",
						"outline-bottom-width", "outline-left-width",
						"outline-top-color", "outline-right-color",
						"outline-bottom-color", "outline-left-color",
						"outline-top-style", "outline-right-style",
						"outline-bottom-style", "outline-left-style"],
				layout : ["position", "display", "visibility", "z-index",
						"overflow-x", "overflow-y", "overflow-clip",
						"white-space", "clip", "float", "clear",
						"-moz-box-sizing"],
				other : ["cursor", "list-style-image", "list-style-position",
						"list-style-type", "marker-offset", "user-focus",
						"user-select", "user-modify", "user-input"]
			};
			...
			Firebug.CSSModule = extend(Firebug.Module, {
				freeEdit : function(styleSheet, value) {
					if (!styleSheet.editStyleSheet) {
						...
						var url = CCSV("@mozilla.org/network/standard-url;1",
								Components.interfaces.nsIURL);
						url.spec = styleSheet.href;
						var editStyleSheet = ownerNode.ownerDocument
								.createElementNS(
						...
						if (ownerNode.hasAttribute("media")) {
							editStyleSheet.setAttribute("media", ownerNode
											.getAttribute("media"))
						}
						ownerNode.parentNode.insertBefore(editStyleSheet,
								ownerNode.nextSibling);
						styleSheet.editStyleSheet = editStyleSheet
					}
					styleSheet.editStyleSheet.innerHTML = value;
					if (FBTrace.DBG_CSS) {
						FBTrace.sysout("css.saveEdit styleSheet.href:"
								+ styleSheet.href + " got innerHTML:" + value
								+ "\n")
					}
					dispatch(this.fbListener, "onCSSFreeEdit", [styleSheet,
									value])
				},
				insertRule : function(styleSheet, cssText, ruleIndex) {
					...
					dispatch(this.fbListeners, "onCSSInsertRule", [styleSheet,
									cssText, ruleIndex]);
					return insertIndex
				},
				deleteRule : function(styleSheet, ruleIndex) {
					if (FBTrace.DBG_CSS) {
						FBTrace.sysout("deleteRule: " + ruleIndex + " "
										+ styleSheet.cssRules.length,
								styleSheet.cssRules)
					}
					dispatch(this.fbListeners, "onCSSDeleteRule", [styleSheet,
									ruleIndex]);
			...
			Firebug.CSSStyleSheetPanel.prototype = extend(
					Firebug.SourceBoxPanel, {
						...
						toggleEditing : function() {
							...
							if (this.editing) {
								...
								var styleSheet = this.location.editStyleSheet
										...
										: this.location;
								var css = getStyleSheetCSS(styleSheet,
										this.context);
								this.stylesheetEditor.styleSheet = this.location;
								Firebug.Editor.startEditing(this.panelNode,
										css, this.stylesheetEditor)
							}
						},
						...
						highlightRule : function(rule) {
							var ruleElement = Firebug.getElementByRepObject(
									this.panelNode.firstChild, rule);
							if (ruleElement) {
								scrollIntoCenterView(ruleElement,
										this.panelNode);
								setClassTimed(ruleElement, "jumpHighlight",
										this.context)
							}
						},
									...
									if (instanceOf(rule, "CSSStyleRule")) {
										...
										if (isIE) {
											selector = selector.replace(
													reSelectorTag, function(s) {
														return s.toLowerCase()
													})
										}
										var ruleId = rule.selectorText + "/"
												+ line;
										rules.push({
						...
						getRuleProperties : function(context, rule, inheritMode) {
							...
							this.addOldProperties(context, ruleId, inheritMode,
									props);
							sortProperties(props);
							return props
						},
						addOldProperties : function(context, ruleId,
								inheritMode, props) {
							if (context.selectorMap
									&& context.selectorMap
											.hasOwnProperty(ruleId)) {
								...
								for (var i = 0; i < moreProps.length; ++i) {
									...
									this.addProperty(prop.name, prop.value,
											prop.important, true, inheritMode,
											props)
								}
							}
						},
						...
						editElementStyle : function() {
							var rulesBox = this.panelNode
									.getElementsByClassName("cssElementRuleContainer")[0];
							var styleRuleBox = rulesBox
									&& Firebug.getElementByRepObject(rulesBox,
											this.selection);
							if (!styleRuleBox) {
								var rule = {
									rule : this.selection,
								...
								if (!rulesBox) {
									styleRuleBox = this.template.cascadedTag
											.replace({
														rules : [rule],
														inherited : [],
														inheritLabel : "Inherited from"
													}, this.panelNode);
									styleRuleBox = styleRuleBox
											.getElementsByClassName("cssElementRuleContainer")[0]
								} else {
									styleRuleBox = this.template.ruleTag
											.insertBefore({
														rule : rule
													}, rulesBox)
								}
								styleRuleBox = styleRuleBox
										.getElementsByClassName("insertInto")[0]
							}
						...
						insertRule : function(row) {
							...
							if (!location) {
								location = getChildByClass(this.panelNode,
										"cssSheet");
						...
						editPropertyRow : function(row) {
							var propValueBox = getChildByClass(row,
									"cssPropValue");
						...
						deletePropertyRow : function(row) {
							...
							var ruleId = Firebug.getRepNode(row)
									.getAttribute("ruleId");
						...
						onMouseDown : function(event) {
							var offset = event.clientX
									- this.panelNode.parentNode.offsetLeft;
						...
						parentPanel : null,
						searchable : true,
						dependents : ["css", "stylesheet", "dom", "domSide",
								"layout"],
						options : {
							hasToolButtons : true
						},
						create : function() {
							...
							if (this.name == "stylesheet") {
								this.onChangeSelect = bind(this.onChangeSelect,
										this);
								var doc = Firebug.browser.document;
								var selectNode = this.selectNode = createElement("select");
								processAllStyleSheets(doc, function(doc,
												styleSheet) {
											var key = StyleSheetCache
													.key(styleSheet);
											var fileName = getFileName(styleSheet.href)
													|| getFileName(doc.location.href);
											...
											option
													.appendChild(Firebug.chrome.document
						...
						initialize : function() {
							...
							if (this.name == "stylesheet") {
								...
								if (styleSheets.length > 0) {
									addEvent(this.selectNode, "change",
											this.onChangeSelect);
						...
						shutdown : function() {
							if (this.name == "stylesheet") {
								removeEvent(this.selectNode, "change",
										this.onChangeSelect)
							}
							this.destroyNode();
							Firebug.Panel.shutdown.apply(this, arguments)
						...
						initializeNode : function(oldPanelNode) {
							addEvent(this.panelNode, "mousedown",
									this.onMouseDown);
							addEvent(this.panelNode, "click", this.onClick)
						},
						destroyNode : function() {
							removeEvent(this.panelNode, "mousedown",
									this.onMouseDown);
						...
						updateLocation : function(styleSheet) {
							...
							var rules = this.getStyleSheetRules(this.context,
									styleSheet);
							var result;
							if (rules.length) {
						...
						getOptionsMenuItems : function() {
							return [{
								...
								command : bindFixed(Firebug.togglePref,
						...
						getContextMenuItems : function(style, target) {
							...
							if (this.infoTipType == "color") {
								...
								if (this.infoTipType == "image") {
									items.push({
												...
												command : bindFixed(openNewTab,
														FBL, this.infoTipObject)
											})
								}
							}
							...
							if (cssRule && hasClass(cssRule, "cssEditableRule")) {
								...
								var propRow = getAncestorByClass(target,
										"cssProp");
								if (propRow) {
									var propName = getChildByClass(propRow,
											"cssPropName")[textContent];
									var isDisabled = hasClass(propRow,
											"disabledStyle");
									items.push({
												label : $STRF("EditProp",
														[propName]),
												nol10n : true,
												command : bindFixed(
														this.editPropertyRow,
														this, propRow)
											}, {
												label : $STRF("DeleteProp",
														[propName]),
												nol10n : true,
												command : bindFixed(
														this.deletePropertyRow,
														this, propRow)
											}, {
												label : $STRF("DisableProp",
														[propName]),
												nol10n : true,
												type : "checkbox",
												checked : isDisabled,
												command : bindFixed(
														this.disablePropertyRow,
														this, propRow)
											})
								}
							}
						...
						showInfoTip : function(infoTip, target, x, y) {
							var propValue = getAncestorByClass(target,
									"cssPropValue");
							if (propValue) {
								var offset = getClientOffset(propValue);
								var offsetX = x - offset.x;
								var text = propValue[textContent];
								var charWidth = propValue.offsetWidth
										/ text.length;
								var charOffset = Math
										.floor(offsetX / charWidth);
								...
								if (cssValue) {
									...
									if (cssValue.type == "rgb"
											|| (!cssValue.type && isColorKeyword(cssValue.value))) {
										this.infoTipType = "color";
										this.infoTipObject = cssValue.value;
										return Firebug.InfoTip
												.populateColorInfoTip(infoTip,
														cssValue.value)
									} else {
										if (cssValue.type == "url") {
											var propNameNode = target.parentNode
													.getElementsByClassName("cssPropName")
													.item(0);
											if (propNameNode
													&& isImageRule(propNameNode[textContent])) {
												var rule = Firebug
														.getRepObject(target);
												var baseURL = this
														.getStylesheetURL(rule);
						...
						getDefaultLocation : function() {
							try {
								...
								if (styleSheets.length) {
									...
									return (Firebug.filterSystemURLs && isSystemURL(getURLForStyleSheet(sheet)))
											? null
											: sheet
								}
							} catch (exc) {
						...
						getObjectDescription : function(styleSheet) {
							...
							if (instance) {
								baseDescription.name = baseDescription.name
										+ " #" + (instance + 1)
							}
						...
						search : function(text, reverse) {
							var curDoc = this.searchCurrentDoc(
									!Firebug.searchGlobal, text, reverse);
						...
						searchOtherDocs : function(text, reverse) {
							...
							function scanDoc(styleSheet) {
								for (var i = 0; i < styleSheet.cssRules.length; i++) {
									if (scanRE
											.test(styleSheet.cssRules[i].cssText)) {
										return true
									}
								}
							}
							if (this.navigateToNextDocument(scanDoc, reverse)) {
								return this.searchCurrentDoc(true, text,
										reverse)
							}
						},
						searchCurrentDoc : function(wrapSearch, text, reverse) {
							...
							if (this.currentSearch
								...
								row = this.currentSearch.findNext(wrapSearch,
										false, reverse, Firebug.Search
												.isCaseSensitive(text))
							} else {
								if (this.editing) {
									this.currentSearch = new TextSearch(this.stylesheetEditor.box);
									row = this.currentSearch.find(text,
											reverse, Firebug.Search
													.isCaseSensitive(text));
									if (row) {
										var sel = this.document.defaultView
												.getSelection();
										sel.removeAllRanges();
							...
							if (row) {
								...
								dispatch([Firebug.A11yModel],
										"onCSSSearchMatchFound", [this, text,
												row]);
								return true
							} else {
								dispatch([Firebug.A11yModel],
			...
			CSSElementPanel.prototype = extend(
					Firebug.CSSStyleSheetPanel.prototype, {
						template : domplate({
							cascadedTag : DIV({
										"class" : "a11yCSSView",
										role : "presentation"
						...
						updateCascadeView : function(element) {
							...
							this
									.getInheritedRules(element, sections,
											usedProps);
						...
						getStyleProperties : function(element, rules,
							...
							var props = this.parseCSSProps(element.style,
									inheritMode);
							this.addOldProperties(this.context,
									getElementXPath(element), inheritMode,
									props);
							sortProperties(props);
							this.markOverridenProps(props, usedProps,
									inheritMode);
						...
						initialize : function() {
							this.context = Firebug.chrome;
							this.document = Firebug.chrome.document;
							Firebug.CSSStyleSheetPanel.prototype.initialize
									.apply(this, arguments);
						...
						removeStateChangeHandlers : function() {
							...
							if (sel) {
								...
								removeEvent(sel, "mousedown",
										this.onStateChange);
								removeEvent(sel, "mouseover",
										this.onStateChange);
			...
			CSSComputedElementPanel.prototype = extend(
					CSSElementPanel.prototype, {
						template : domplate({
							computedTag : DIV({
								"class" : "a11yCSSView",
								role : "list",
						...
						updateComputedView : function(element) {
							var win = isIE
									...
									: element.ownerDocument.defaultView;
							var style = isIE ? element.currentStyle : win
									.getComputedStyle(element, "");
							var groups = [];
					...
					if (hasClass(target, "cssPropName")) {
						if (value && previousValue != value) {
							...
							if (propValue && propValue != "undefined") {
								if (FBTrace.DBG_CSS) {
									FBTrace.sysout("CSSEditor.saveEdit : "
											+ previousValue + "->" + value
											+ " = " + propValue + "\n")
								}
								if (previousValue) {
						...
						if (getAncestorByClass(target, "cssPropValue")) {
							...
							if (FBTrace.DBG_CSS) {
								FBTrace
										.sysout("CSSEditor.saveEdit propName=propValue: "
												+ propName
												+ " = "
												+ propValue
												+ "\n")
			...
			CSSRuleEditor.prototype = domplate(Firebug.InlineEditor.prototype,
					{
						...
						saveEdit : function(target, value, previousValue) {
							if (FBTrace.DBG_CSS) {
								FBTrace.sysout("CSSRuleEditor.saveEdit: '"
												+ value + "'  '"
							...
							if (rule || Firebug.getRepObject(row.nextSibling)) {
								var searchRule = rule
										|| Firebug
												.getRepObject(row.nextSibling);
								for (ruleIndex = 0; ruleIndex < cssRules.length
										&& searchRule != cssRules[ruleIndex]; ruleIndex++) {
								}
							}
							if (oldRule) {
								Firebug.CSSModule.deleteRule(styleSheet,
										ruleIndex)
							}
							if (value) {
								var cssText = [value, "{",];
								var props = row
										.getElementsByClassName("cssProp");
								for (var i = 0; i < props.length; i++) {
									var propEl = props[i];
									if (!hasClass(propEl, "disabledStyle")) {
										cssText.push(getChildByClass(propEl,
												"cssPropName")[textContent]);
										cssText.push(":");
										cssText.push(getChildByClass(propEl,
												"cssPropValue")[textContent]);
										cssText.push(";")
									}
								}
								cssText.push("}");
								...
								try {
									var insertLoc = Firebug.CSSModule
											.insertRule(styleSheet, cssText,
													ruleIndex);
									rule = cssRules[insertLoc];
									ruleIndex++
								} catch (err) {
									if (FBTrace.DBG_CSS || FBTrace.DBG_ERRORS) {
										FBTrace.sysout("CSS Insert Error: "
														+ err, err)
									}
							...
							row.repObject = rule;
							if (!oldRule) {
								var ruleId = "new/" + value + "/"
										+ (++CSSRuleEditor.uniquifier);
								row.setAttribute("ruleId", ruleId)
							}
							this.panel
									.markChange(this.panel.name == "stylesheet")
						}
					});
			...
			StyleSheetEditor.prototype = domplate(Firebug.BaseEditor, {
						...
						show : function(target, panel, value, textSize,
							...
							this.input.focus();
							var command = Firebug.chrome
									.$("cmd_toggleCSSEditing");
	...
	FBL.ns(function() {
		with (FBL) {
			Firebug.Script = extend(Firebug.Module, {
						getPanel : function() {
							return Firebug.chrome ? Firebug.chrome
									.getPanel("Script") : null
						},
			...
			ScriptPanel.prototype = extend(Firebug.Panel, {
				...
				create : function() {
					...
					for (var i = 0, script; script = scripts[i]; i++) {
						...
						option.appendChild(Firebug.chrome.document
								.createTextNode(fileName));
						selectNode.appendChild(option)
					}
					this.toolButtonsNode.appendChild(selectNode)
		...
		with (FBL) {
			...
			Firebug.DOMBasePanel.prototype = extend(Firebug.Panel, {
				...
				showMembers : function(members, update, scrollTop) {
					...
					var priorScrollTop = scrollTop == undefined
							? panelNode.scrollTop
							: scrollTop;
					...
					if (offscreen) {
						timeouts.push(this.context.setTimeout(function() {
									if (panelNode.firstChild) {
										panelNode.replaceChild(table,
												panelNode.firstChild)
									} else {
						...
						timeouts.push(this.context.setTimeout(function() {
									panelNode.scrollTop = scrollTop == undefined
											? 0
											: scrollTop
								}, delay))
					}
				...
				getRowPropertyValue : function(row) {
					...
					if (object) {
						...
						if (object instanceof jsdIStackFrame) {
							return Firebug.Debugger.evaluate(propName,
									this.context)
						} else {
				...
				ishow : function(state) {
					if (this.context.loaded && !this.selection) {
						...
						var selectObject = defaultObject = this
								.getDefaultSelection(this.context);
						...
						if (this.propertyPath.length > 1) {
							for (var i = 1; i < this.propertyPath.length; ++i) {
								...
								if (selectObject) {
									this.objectPath.push(new Property(object,
											name))
								} else {
									...
									selectObject = this
											.getPathObject(this.objectPath.length
													- 1);
									break
								}
							}
				...
				updateSelection : function(object) {
					...
					var previousView = previousIndex == -1
							? null
							: this.viewPath[previousIndex];
					...
					if (newPath || pathIndex == -1) {
						...
						if (newPath) {
							if (previousView) {
								...
								var start = previousIndex + 1, length = this.objectPath.length
										- start;
							...
							if (!value) {
								if (FBTrace.DBG_ERRORS) {
									FBTrace
											.sysout("dom.updateSelection no pathObject for "
													+ previousIndex + "\n")
								}
								return
							}
							for (var i = 0, length = newPath.length; i < length; ++i) {
								...
								try {
									...
									if (FBTrace.DBG_ERRORS) {
										FBTrace
												.sysout("dom.updateSelection FAILS at path_i="
														+ i
														+ " for name:"
														+ name + "\n")
									}
			...
			DOMMainPanel.prototype = extend(Firebug.DOMBasePanel.prototype, {
				...
				create : function() {
					Firebug.DOMBasePanel.prototype.create
							.apply(this, arguments);
					this.onClick = bind(this.onClick, this);
				...
				initialize : function(oldPanelNode) {
					Firebug.DOMBasePanel.prototype.initialize.apply(this,
							arguments);
					addEvent(this.panelNode, "click", this.onClick);
					this.ishow();
				...
				shutdown : function() {
					removeEvent(this.panelNode, "click", this.onClick);
					Firebug.DOMBasePanel.prototype.shutdown.apply(this,
							arguments)
				}
			});
			...
			var getMembers = function getMembers(object, level) {
				...
				try {
					...
					if (isIE && isFunction(object)) {
						addMember("user", userProps, "prototype",
					...
					for (var name in insecureObject) {
						...
						try {
							...
							if (FBTrace.DBG_ERRORS && FBTrace.DBG_DOM) {
								FBTrace.sysout("dom.getMembers cannot access "
												+ name, exc)
							}
						}
						...
						if (ordinal || ordinal == 0) {
							...
							if (isFunction(val)) {
								if (isClassFunction(val)) {
									addMember("userClass", userClasses, name,
											val, level)
								} else {
									if (name in domMembers) {
										addMember("domFunction", domFuncs,
												name, val, level,
												domMembers[name])
									} else {
										addMember("userFunction", userFuncs,
												name, val, level)
									}
								}
							} else {
								var prefix = "";
								if (name in domMembers) {
									addMember("dom", domProps, (prefix + name),
											val, level, domMembers[name])
								} else {
									if (name in domConstantMap) {
										addMember("dom", domConstants,
												(prefix + name), val, level)
									} else {
										addMember("user", userProps,
												(prefix + name), val, level)
									}
								}
							}
			...
			function expandMembers(members, toggles, offset, level) {
				...
				for (var i = offset; i < members.length; ++i) {
					...
					if (toggles.hasOwnProperty(member.name)) {
						...
						i += newMembers.length
								+ expandMembers(members, toggles[member.name],
										i + 1, level + 1)
					}
				}
				return expanded
			...
			var addMember = function addMember(type, props, name, value, level,
					order) {
				var rep = Firebug.getRep(value);
				var tag = rep.shortTag ? rep.shortTag : rep.tag;
				...
				var hasChildren = hasProperties(value)
						...
						&& (isFunction(value)
								|| (valueType == "object" && value != null) || (valueType == "string" && value.length > Firebug.stringCropLength));
				props.push({
							name : name,
							value : value,
							type : type,
	...
	(function() {
		...
		this.dumpProperties = function(title, object) {
			return this.logFormatted("dumpProperties() not supported.",
					"warning")
		};
		...
		this.writeMessage = function(message, className) {
			...
			var isScrolledToBottom = container.scrollTop
		...
		this.writeRow = function(message, className) {
			var row = this.getPanel().contentNode.ownerDocument
					.createElement("div");
	...
	FBL.ns(function() {
				with (FBL) {
					...
					TracePanel.prototype = extend(Firebug.Panel, {
								...
								initialize : function() {
									Firebug.Panel.initialize.apply(this,
											arguments);
									this.clearButton.initialize()
								}
							});
