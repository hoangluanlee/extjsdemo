 ...
 * 
 * (c) 2009-2010 Torstein Hønsi Embedded license information at
 * <http://www.fusioncharts.com/license> All modifications made to this code are
 * copyright of FusionCharts Technologies LLP. License:
 ...
 */
(function() {
	var u = document, win = window, math = Math, mathRound = math.round, mathFloor = math.floor, mathCeil = math.ceil, mathMax = math.max, mathMin = math.min, mathAbs = math.abs, mathCos = math.cos, mathSin = math.sin, userAgent = navigator.userAgent, isIE = /msie/i
			.test(userAgent)
			&& !win.opera, isWebKit = /AppleWebKit/.test(userAgent), hasSVG = win.SVGAngle
			|| u.implementation.hasFeature(
					"http://www.w3.org/TR/SVG11/feature#BasicStructure", "1.1"), colorCounter, symbolCounter, symbolSizes = {}, idCounter = 0, timeFactor = 1, garbageBin, defaultOptions, dateFormat, UNDEFINED, DIV = 'div', ABSOLUTE = 'absolute', RELATIVE = 'relative', HIDDEN = 'hidden', PREFIX = 'highcharts-', VISIBLE = 'visible', PX = 'px', NONE = 'none', M = 'M', L = 'L', TRACKER_FILL = 'rgba(192,192,192,'
			+ (hasSVG ? 0.000001 : 0.002) + ')', NORMAL_STATE = '', HOVER_STATE = 'hover', SELECT_STATE = 'select', makeTime, getMinutes, getHours, getDay, getDate, getMonth, getFullYear, setMinutes, setHours, setDate, setMonth, setFullYear, globalAdapter = win.HighchartsAdapter, adapter = globalAdapter
			|| {}, each = adapter.each, grep = adapter.grep, map = adapter.map, merge = adapter.merge, hyphenate = adapter.hyphenate, addEvent = adapter.addEvent, removeEvent = adapter.removeEvent, fireEvent = adapter.fireEvent, animate = adapter.animate, stop = adapter.stop, getAjax = adapter.getAjax, seriesTypes = {};
	function extend(a, b) {
		if (!a) {
			a = {}
		}
	...
	function setTimeMethods() {
		...
		makeTime = g ? Date.UTC : function(a, b, c, d, e, f) {
			return new Date(a, b, pick(c, 1), pick(d, 0), pick(e, 0),
					pick(f, 0)).getTime()
		};
	...
	var C = function(c) {
		var d = [], result;
		function init(a) {
			if ((result = /rgba\(\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]{1,3})\s*,\s*([0-9]?(?:\.[0-9]+)?)\s*\)/
					.exec(a))) {
				d = [parseInt(result[1], 10), parseInt(result[2], 10),
						parseInt(result[3], 10), parseFloat(result[4], 10)]
			} else if ((result = /#([a-fA-F0-9]{2})([a-fA-F0-9]{2})([a-fA-F0-9]{2})/
					.exec(a))) {
				d = [parseInt(result[1], 16), parseInt(result[2], 16),
	...
	function numberFormat(a, b, e, f) {
		var g = defaultOptions.lang, n = a, c = isNaN(b = mathAbs(b)) ? 2 : b, d = e === undefined
				? g.decimalPoint
				: e, t = f === undefined ? g.thousandsSep : f, s = n < 0
				? "-"
				: "", i = parseInt(n = mathAbs(+n || 0).toFixed(c), 10) + "", j = (j = i.length) > 3
				? j % 3
				: 0;
	...
	SVGElement.prototype = {
		...
		updateTransform : function() {
			var a = this, translateX = a.translateX || 0, translateY = a.translateY
					|| 0, inverted = a.inverted, transform = [];
			if (inverted) {
		...
		add : function(a) {
			var b = this.renderer, parentWrapper = a || b, parentNode = parentWrapper.element
					|| b.box, childNodes = parentNode.childNodes, element = this.element, zIndex = attr(
			...
			if (parentWrapper.handleZ) {
				for (i = 0; i < childNodes.length; i++) {
					...
					if (otherElement != element
							&& (parseInt(otherZIndex, 10) > zIndex || (!defined(zIndex) && defined(otherZIndex)))) {
		...
		shadow : function(a) {
			var b = [], i, shadow, element = this.element, transform = this.parentInverted
					? '(-1,-1)'
					: '(1,1)';
			if (a) {
				for (i = 1; i <= 3; i++) {
	...
	D.prototype = {
		...
		buildText : function(f, g) {
			var h = g.toString().replace(/<(b|strong)>/g,
					'<span style="font-weight:bold">').replace(/<(i|em)>/g,
					'<span style="font-style:italic">').replace(/<a/g, '<span')
					.replace(/<\/(b|strong|i|em|a)>/g, '</span>')
					.split(/<br[^>]?>/g), childNodes = f.childNodes, styleRegex = /style="([^"]+)"/, hrefRegex = /href="([^"]+)"/, parentX = attr(
					f, 'x'), i = childNodes.length;
			while (i--) {
				f.removeChild(childNodes[i])
			}
			each(h, function(c, d) {
						var e, spanNo = 0;
						c = c.replace(/<span/g, '|||<span').replace(
								/<\/span>/g, '</span>|||');
						e = c.split('|||');
						each(e, function(a) {
									if (a !== '' || e.length == 1) {
										var b = {}, tspan = u.createElementNS(
		...
		symbol : function(b, x, y, c, d) {
			var e, symbolFn = this.symbols[b], path = symbolFn
					&& symbolFn(x, y, c, d), imageRegex = /^url\((.*?)\)$/, imageSrc;
			if (path) {
				e = this.path(path);
				extend(e, {
							symbolName : b,
				...
				createElement('img', {
							onload : function() {
								var a = this, size = symbolSizes[a.src]
										|| [a.width, a.height];
								e.attr({
		...
		symbols : {
			'square' : function(x, y, a) {
				var b = 0.707 * a;
				return [M, x - b, y - b, L, x + b, y - b, x + b, y + b, x - b,
						y + b, 'Z']
			},
			'triangle' : function(x, y, a) {
				return [M, x, y - 1.33 * a, L, x + a, y + 0.67 * a, x - a,
						y + 0.67 * a, 'Z']
			},
			...
			'arc' : function(x, y, a, b) {
				var c = Math.PI, start = b.start, end = b.end - 0.000001, innerRadius = b.innerR, cosStart = mathCos(start), sinStart = mathSin(start), cosEnd = mathCos(end), sinEnd = mathSin(end), longArc = b.end
						- start < c ? 0 : 1;
				return [M, x + a * cosStart, y + a * sinStart, 'A', a, a, 0,
						longArc, 1, x + a * cosEnd, y + a * sinEnd, L,
						x + innerRadius * cosEnd, y + innerRadius * sinEnd,
						'A', innerRadius, innerRadius, 0, longArc, 0,
						x + innerRadius * cosStart, y + innerRadius * sinStart,
						'Z']
			}
		},
		clipRect : function(x, y, a, b) {
			var c, id = PREFIX + idCounter++, clipPath = this
					.createElement('clipPath').attr({
								id : id
							}).add(this.defs);
		...
		color : function(b, c, d) {
			...
			if (b && b.linearGradient) {
				var f = this, strLinearGradient = 'linearGradient', linearGradient = b[strLinearGradient], id = PREFIX
	...
	if (!hasSVG) {
		var F = extendClass(SVGElement, {
			init : function(a, b) {
				var c = ['<', b, ' filled="f" stroked="f"'], style = [
						'position: ', ABSOLUTE, ';'];
				if (b == 'shape' || b == DIV) {
			...
			add : function(a) {
				var b = this, renderer = b.renderer, element = b.element, box = renderer.box, inverted = a
						&& a.inverted, parentStyle, parentNode = a ? a.element
						|| a : box;
				if (inverted) {
				...
				if (typeof b == 'string') {
					...
					for (d in b) {
						...
						if (symbolName
							...
							elemStyle[d] = value;
							if (documentMode == 8 && d == 'visibility'
									&& nodeName == 'DIV') {
		...
		E.prototype = merge(D.prototype, {
			...
			init : function(a, b, c) {
				...
				if (!u.namespaces.hcv) {
					...
					u.createStyleSheet().cssText = 'hcv\\:fill, hcv\\:path, hcv\\:textpath, hcv\\:shape, hcv\\:stroke, hcv\\:line '
			...
			clipRect : function(x, y, c, d) {
				...
				return extend(e, {
					...
					getCSS : function(a) {
						var b = e.element.style, top = b.top, left = b.left, right = left
								+ b.width, bottom = top + b.height, ret = {
							clip : 'rect(' + (a ? left : top) + 'px,'
									+ (a ? bottom : right) + 'px,'
									+ (a ? right : bottom) + 'px,'
									+ (a ? top : left) + 'px)'
						};
			...
			color : function(b, c, d) {
				...
				if (b && b.linearGradient) {
					...
					angle = 90
							- math.atan((linearGradient[3] - linearGradient[1])
									/ (linearGradient[2] - linearGradient[0]))
							* 180 / math.PI;
					markup = ['<', d, ' colors="0% ', color1, ',100% ', color2,
							'" angle="', angle, '" opacity="', opacity2,
							'" o:opacity2="', opacity1,
			...
			text : function(a, x, y, b, c, d) {
				...
				d = d || 'left';
				c = c || 0;
				var e, elem, spanWidth, lineHeight = mathRound(parseInt(
						b.fontSize || 12, 10)
						* 1.2), defaultChartStyle = defaultOptions.chart.style;
				x = mathRound(x);
				y = mathRound(y);
				extend(b, {
				...
				if (!c) {
					...
					var f = (c || 0) * math.PI * 2 / 360, costheta = mathCos(f), sintheta = mathSin(f), length = 10, baselineCorrection = lineHeight
							* 0.3, left = d == 'left', right = d == 'right', x1 = left
							? x
							: x - length * costheta, x2 = right ? x : x
							+ length * costheta, y1 = left ? y : y - length
							* sintheta, y2 = right ? y : y + length * sintheta;
					...
					createElement('<hcv:textpath style="v-text-align:' + d
									+ ';' + serializeCSS(b).replace(/"/g, "'")
									+ '" on="true" string="'
									+ a.toString().replace(/<br[^>]?>/g, '\n')
									+ '">', null, null, elem)
				}
				return e
			},
			...
			symbols : {
				arc : function(x, y, a, b) {
					var c = b.start, optionsEnd = b.end, end = optionsEnd - c == 2
							* Math.PI ? optionsEnd - 0.001 : optionsEnd, cosStart = mathCos(c), sinStart = mathSin(c), cosEnd = mathCos(end), sinEnd = mathSin(end), innerRadius = b.innerR;
					if (optionsEnd - c === 0) {
						return ['x']
					}
					return ['wa', x - a, y - a, x + a, y + a, x + a * cosStart,
							y + a * sinStart, x + a * cosEnd, y + a * sinEnd,
							'at', x - innerRadius, y - innerRadius,
							x + innerRadius, y + innerRadius,
							x + innerRadius * cosEnd, y + innerRadius * sinEnd,
							x + innerRadius * cosStart,
							y + innerRadius * sinStart, 'x', 'e']
				},
				circle : function(x, y, r) {
					return ['wa', x - r, y - r, x + r, y + r, x + r, y, x + r,
							y, 'e']
				},
				rect : function(a, b, r, c) {
					var d = c.width, height = c.height, right = a + d, bottom = b
							+ height;
					r = mathMin(r, d, height);
					return [M, a + r, b, L, right - r, b, 'wa', right - 2 * r,
							b, right, b + 2 * r, right - r, b, right, b + r, L,
							right, bottom - r, 'wa', right - 2 * r,
							bottom - 2 * r, right, bottom, right, bottom - r,
							right - r, bottom, L, a + r, bottom, 'wa', a,
							bottom - 2 * r, a + 2 * r, bottom, a + r, bottom,
							a, bottom - r, L, a, b + r, 'wa', a, b, a + 2 * r,
							b + 2 * r, a, b + r, a + r, b, 'x', 'e']
				}
			}
		})
	}
	...
	function Chart(n, o) {
		...
		var p = n.chart, optionsMargin = p.margin, margin = typeof optionsMargin == 'number'
				? [optionsMargin, optionsMargin, optionsMargin, optionsMargin]
				: optionsMargin, plotTop = pick(p.marginTop, margin[0]), marginRight = pick(
				p.marginRight, margin[1]), marginBottom = pick(p.marginBottom,
				margin[2]), plotLeft = pick(p.marginLeft, margin[3]), renderTo, renderToClone, container, containerId, chartWidth, chartHeight, chart = this, chartEvents = p.events, eventType, getAlignment, isInsidePlot, tooltip, mouseIsDown, loadingLayer, loadingShown, plotHeight, plotWidth, plotSizeX, plotSizeY, tracker, trackerGroup, legend, position, hasCartesianSeries = p.showAxes, axes = [], maxTicks, series = [], inverted, renderer, tooltipTick, tooltipInterval, zoom, zoomOut;
		function Axis(j, k) {
			...
			k = merge(l ? A : defaultYAxisOptions, horiz ? (opposite
							? defaultTopAxisOptions
							: defaultBottomAxisOptions) : (opposite
							? defaultRightAxisOptions
							: defaultLeftAxisOptions), k);
			var m = this, isDatetimeAxis = k.type == 'datetime', offset = k.offset
					|| 0, xOrY = l ? 'x' : 'y', axisLength = horiz
					? plotWidth
					: plotHeight, transA, transB = horiz
					? plotLeft
					: marginBottom, axisGroup, gridGroup, dataMin, dataMax, associatedSeries, userSetMin, userSetMax, max = null, min = null, minPadding = k.minPadding, maxPadding = k.maxPadding, isLinked = defined(k.linkedTo), ignoreMinPadding, ignoreMaxPadding, usePercentage, events = k.events, eventType, plotBands = k.plotBands
					|| [], plotLines = k.plotLines || [], tickInterval, minorTickInterval, magnitude, tickPositions, tickAmount, dateTimeLabelFormat, labelFormatter = k.labels.formatter
					|| function() {
						var a = this.value;
						return dateTimeLabelFormat ? dateFormat(
								dateTimeLabelFormat, a) : a
					}, categories = k.categories || (l && j.columnCount), reversed = k.reversed, tickmarkOffset = (categories && k.tickmarkPlacement == 'between')
					? 0.5
					: 0;
			function getSeriesExtremes() {
				...
				each(series, function(d) {
					...
					each(['xAxis', 'yAxis'], function(a) {
						if (d.isCartesian
								&& (a == 'xAxis' && l || a == 'yAxis' && !l)
								&& ((d.options[a] == k.index) || (d.options[a] === UNDEFINED && k.index === 0))) {
							d[a] = m;
			...
			function drawPlotLine(a, b, c) {
				if (c) {
					...
					if (!skip) {
						renderer.path(renderer.crispLine([M, d, y1, L, x2, y2],
								c)).attr({
									stroke : b,
									'stroke-width' : c
								}).add(gridGroup)
			...
			function addTick(a, b, c, d, e, f, g) {
				...
				if (d) {
					renderer.path(renderer.crispLine([M, h, y1, L, x2, y2], d))
							.attr({
										stroke : c,
										'stroke-width' : d
									}).add(axisGroup)
				...
				if (f && labelOptions.enabled) {
					str = labelFormatter.call({
								...
								isLast : a == tickPositions[tickPositions.length
										- 1],
								dateTimeLabelFormat : dateTimeLabelFormat,
								value : (categories && categories[a]
										? categories[a]
										: a)
							});
					if (str || str === 0) {
						...
						y1 = y1
								...
								- (tickmarkOffset && !horiz ? tickmarkOffset
										* transA * (reversed ? 1 : -1) : 0);
						renderer.text(str, h, y1, labelOptions.style,
								labelOptions.rotation, labelOptions.align)
								.add(axisGroup)
					}
				}
			}
			function normalizeTickInterval(a, b) {
				var c;
				magnitude = b ? 1 : math.pow(10, mathFloor(math.log(a)
								/ math.LN10));
				c = a / magnitude;
				if (!b) {
					b = [1, 2, 2.5, 5, 10];
			...
			function setDateTimeTickPositions() {
				tickPositions = [];
				var i, useUTC = defaultOptions.global.useUTC, oneSecond = 1000
						/ timeFactor, oneMinute = 60000 / timeFactor, oneHour = 3600000
						/ timeFactor, oneDay = 24 * 3600000 / timeFactor, oneWeek = 7
						* 24 * 3600000 / timeFactor, oneMonth = 30 * 24
						* 3600000 / timeFactor, oneYear = 31556952000
						/ timeFactor, units = [
						['second', oneSecond, [1, 2, 5, 10, 15, 30]],
						['minute', oneMinute, [1, 2, 5, 10, 15, 30]],
						['hour', oneHour, [1, 2, 3, 4, 6, 8, 12]],
						['day', oneDay, [1, 2]], ['week', oneWeek, [1, 2]],
						['month', oneMonth, [1, 2, 3, 4, 6]],
						['year', oneYear, null]], unit = units[6], interval = unit[1], multiples = unit[2];
				for (i = 0; i < units.length; i++) {
					unit = units[i];
					interval = unit[1];
					multiples = unit[2];
					if (units[i + 1]) {
						var a = (interval * multiples[multiples.length - 1] + units[i
								+ 1][1])
								/ 2;
						if (tickInterval <= a) {
							break
				...
				var b = normalizeTickInterval(tickInterval / interval,
						multiples), minYear, minDate = new Date(min
						* timeFactor);
				minDate.setMilliseconds(0);
				if (interval >= oneSecond) {
					minDate.setSeconds(interval >= oneMinute ? 0 : b
							* mathFloor(minDate.getSeconds() / b))
				}
				if (interval >= oneMinute) {
					minDate[setMinutes](interval >= oneHour ? 0 : b
							* mathFloor(minDate[getMinutes]() / b))
				}
				if (interval >= oneHour) {
					minDate[setHours](interval >= oneDay ? 0 : b
							* mathFloor(minDate[getHours]() / b))
				}
				if (interval >= oneDay) {
					minDate[setDate](interval >= oneMonth ? 1 : b
							* mathFloor(minDate[getDate]() / b))
				}
				if (interval >= oneMonth) {
					minDate[setMonth](interval >= oneYear ? 0 : b
							* mathFloor(minDate[getMonth]() / b));
				...
				while (c < max && i < plotWidth) {
					...
					if (interval == oneYear) {
						...
						c = makeTime(minYear, minMonth, minDateDate + i * b
										* (interval == oneDay ? 1 : 7))
					} else {
						c += interval * b
					}
			...
			function setLinearTickPositions() {
				var i, roundedMin = mathFloor(min / tickInterval)
						* tickInterval, roundedMax = mathCeil(max
						/ tickInterval)
						* tickInterval;
				tickPositions = [];
			...
			function setTickPositions() {
				...
				var a = tickPositions[0], roundedMax = tickPositions[tickPositions.length
						- 1];
				if (k.startOnTick) {
					min = a
				} else if (min > a) {
				...
				if (!isDatetimeAxis && !categories) {
					...
					if (calculatedTickAmount < tickAmount) {
						while (tickPositions.length < tickAmount) {
							tickPositions
									.push(correctFloat(tickPositions[tickPositions.length
											- 1]
											+ tickInterval))
						}
			...
			function setScale() {
				...
				if (isLinked) {
					var b = j[l ? 'xAxis' : 'yAxis'][k.linkedTo], linkedParentExtremes = b
							.getExtremes();
				...
				if (max - min < maxZoom) {
					...
					min = mathMax(min - zoomOffset, pick(k.min, min
											- zoomOffset));
				...
				if (!categories && !usePercentage && !isLinked && defined(min)
						&& defined(max)) {
					a = (max - min) || 1;
					if (!defined(k.min) && !defined(userSetMin) && minPadding
							&& (dataMin < 0 || !ignoreMinPadding)) {
						min -= a * minPadding
					}
					if (!defined(k.max) && !defined(userSetMax) && maxPadding
				...
				if (categories || min == max) {
					...
					tickInterval = pick(k.tickInterval, (max - min)
									* k.tickPixelInterval / axisLength)
				}
				if (!isDatetimeAxis && !defined(k.tickInterval)) {
					tickInterval = normalizeTickInterval(tickInterval)
				}
				minorTickInterval = k.minorTickInterval === 'auto'
						&& tickInterval
						? tickInterval / 5
						: k.minorTickInterval;
				setTickPositions();
			...
			function render() {
				var c = k.title, alternateGridColor = k.alternateGridColor, minorTickWidth = k.minorTickWidth, lineWidth = k.lineWidth, lineLeft, lineTop, tickmarkPos, hasData = associatedSeries.length
				...
				if (hasData || isLinked) {
					...
					if (minorTickInterval && !categories) {
						for (var i = min; i <= max; i += minorTickInterval) {
							drawPlotLine(i, k.minorGridLineColor,
									k.minorGridLineWidth);
							if (minorTickWidth) {
								addTick(i, k.minorTickPosition,
										k.minorTickColor, minorTickWidth,
										k.minorTickLength)
							}
						}
					}
					each(tickPositions, function(a, b) {
						...
						drawPlotLine(tickmarkPos, k.gridLineColor,
								k.gridLineWidth);
						addTick(
								a,
								k.tickPosition,
								...
								k.tickWidth,
								k.tickLength,
								!((a == min && !k.showFirstLabel) || (a == max && !k.showLastLabel)),
								b)
					});
				...
				if (!m.hasRenderedTitle && !m.axisTitle && c && c.text) {
					...
					var f = (horiz ? plotTop + plotHeight : plotLeft)
							+ (horiz ? 1 : -1)
							* (opposite ? -1 : 1)
							* c.margin
							- (isIE
									? parseInt(c.style.fontSize || 12, 10) / 3
									: 0);
					m.axisTitle = renderer.text(
							c.text,
							(horiz ? e : f + (opposite ? plotWidth : 0)
									+ offset)
									+ (c.x || 0),
							(horiz
									? f - (opposite ? plotHeight : 0) + offset
									: e)
									+ (c.y || 0), c.style, c.rotation || 0, {
								low : 'left',
								middle : 'center',
								high : 'right'
							}[c.align]).attr({
		...
		function Toolbar(f) {
			...
			function add(a, b, c, d) {
				if (!g[a]) {
					var e = renderer.text(b, plotLeft + plotWidth - 20,
							plotTop + 30, n.toolbar.itemStyle, 0, 'right').on(
							'click', d).attr({
								zIndex : 20
							}).add();
					g[a] = e
		...
		function Tooltip(c) {
			var d, borderWidth = c.borderWidth, style = c.style, padding = parseInt(
					style.padding, 10), boxOffLeft = borderWidth + padding, tooltipIsHidden = true, boxWidth, boxHeight, currentX = 0, currentY = 0;
			style.padding = 0;
			var e = renderer.g('tooltip').attr({
						zIndex : 8
					}).add(), box = renderer.rect(boxOffLeft, boxOffLeft, 0, 0,
					c.borderRadius, borderWidth).attr({
						fill : c.backgroundColor,
						'stroke-width' : borderWidth
					}).add(e).shadow(c.shadow), label = renderer.text('',
					padding + boxOffLeft,
					parseInt(style.fontSize, 10) + padding + boxOffLeft).attr({
						zIndex : 1
					}).css(style).add(e);
			function move(a, b) {
			...
			function refresh(a) {
				var b = a.series, borderColor = c.borderColor || a.color
						|| b.color || '#606060', x, y, boxX, boxY, show, bBox, text = a.tooltipText, tooltipPos = a.tooltipPos;
				d = b;
				x = mathRound(tooltipPos ? tooltipPos[0] : (inverted
						? plotWidth - a.plotY
						: a.plotX));
				y = mathRound(tooltipPos ? tooltipPos[1] : (inverted
						? plotHeight - a.plotX
						: a.plotY));
		...
		function MouseTracker(f, g) {
			var h, mouseDownY, hasDragged, selectionMarker, zoomType = p.zoomType, zoomX = /x/
					.test(zoomType), zoomY = /y/.test(zoomType), zoomHor = zoomX
					&& !inverted || zoomY && inverted, zoomVert = zoomY
					&& !inverted || zoomX && inverted;
			...
			function getMouseCoordinates(e) {
				...
				each(axes, function(a, i) {
					var b = a.translate, isXAxis = a.isXAxis, isHorizontal = inverted
							? !isXAxis
							: isXAxis;
					c[isXAxis ? 'xAxis' : 'yAxis'].push({
						axis : a,
						value : b(isHorizontal
										? e.chartX - plotLeft
										: plotHeight - e.chartY + plotTop, true)
					})
				});
				return c
			}
			function onmousemove(e) {
				...
				if (hoverSeries && hoverSeries.tracker) {
					a = hoverSeries.tooltipPoints[inverted
							? e.chartY
							: e.chartX - plotLeft];
					if (a && a != hoverPoint) {
						a.onMouseOver()
				...
				if (selectionMarker) {
					...
					if (hasDragged) {
						each(axes, function(a, i) {
							var b = a.translate, isXAxis = a.isXAxis, isHorizontal = inverted
									? !isXAxis
									: isXAxis, selectionMin = b(isHorizontal
											? selectionLeft
											: plotHeight - selectionTop
													- selectionBox.height, true), selectionMax = b(
									isHorizontal ? selectionLeft
											+ selectionBox.width : plotHeight
											- selectionTop, true);
							c[isXAxis ? 'xAxis' : 'yAxis'].push({
										axis : a,
			...
			function setDOMEvents() {
				...
				container.onmousedown = function(e) {
					...
					if (hasCartesianSeries && (zoomX || zoomY)) {
						if (!selectionMarker) {
							selectionMarker = renderer.rect(plotLeft, plotTop,
									zoomHor ? 1 : plotWidth,
				...
				container.onmousemove = function(e) {
					e = normalizeMouseEvent(e);
					e.returnValue = false;
					var a = e.chartX, chartY = e.chartY, isOutsidePlot = !isInsidePlot(
							a - plotLeft, chartY - plotTop);
					if (mouseIsDown) {
						hasDragged = Math.sqrt(Math.pow(h - a, 2)
								+ Math.pow(mouseDownY - chartY, 2)) > 10;
						if (zoomHor) {
							var b = a - h;
				...
				container.onclick = function(e) {
					...
					if (!hasDragged) {
						if (a && attr(e.target, 'isTracker')) {
							...
							fireEvent(f.hoverSeries || a.series, 'click',
									extend(e, {
												point : a
											}));
		...
		var q = function(e) {
			...
			var g = f.layout == 'horizontal', symbolWidth = f.symbolWidth, symbolPadding = f.symbolPadding, allItems = [], style = f.style, itemStyle = f.itemStyle, itemHoverStyle = f.itemHoverStyle, itemHiddenStyle = f.itemHiddenStyle, padding = parseInt(
					style.padding, 10), rightPadding = 20, lineHeight = f.lineHeight
					|| 16, y = 18, initialItemX = 4 + padding + symbolWidth
					+ symbolPadding, itemX, itemY, lastItemY, box, legendBorderWidth = f.borderWidth, legendBackgroundColor = f.backgroundColor, legendGroup, offsetWidth, widthOption = f.width, boxWidth, boxHeight, series = e.series, reversedLegend = f.reversed;
			function colorizeItem(a, b) {
				var c = a.legendItem, legendLine = a.legendLine, legendSymbol = a.legendSymbol, hiddenColor = itemHiddenStyle.color, textColor = b
						? f.itemStyle.color
						: hiddenColor, symbolColor = b ? a.color : hiddenColor;
				if (c) {
					c.css({
								color : textColor
							})
			...
			function renderItem(c) {
				var d, itemWidth, legendSymbol, simpleSymbol, li = c.legendItem, series = c.series
						|| c;
				if (!li) {
					simpleSymbol = /^(bar|pie|area|column)$/.test(series.type);
					c.legendItem = li = renderer.text(f.labelFormatter.call(c),
							0, 0).css(c.visible ? itemStyle : itemHiddenStyle)
					...
					if (!simpleSymbol && c.options && c.options.lineWidth) {
						c.legendLine = renderer.path([M,
								-symbolWidth - symbolPadding, 0, L,
								-symbolPadding, 0]).attr({
					...
					if (simpleSymbol) {
						legendSymbol = renderer.rect(
								-symbolWidth - symbolPadding, -11, symbolWidth,
								12, 2).attr({
									'stroke-width' : 0,
									zIndex : 3
								}).add(legendGroup)
					} else if (c.options && c.options.marker
							&& c.options.marker.enabled) {
						legendSymbol = renderer.symbol(c.symbol,
								-symbolWidth / 2 - symbolPadding, -4,
								c.options.marker.radius)
								.attr(c.pointAttr[NORMAL_STATE]).attr({
											zIndex : 3
										}).add(legendGroup)
					}
				...
				d = li.getBBox();
				lastItemY = itemY;
				c.legendItemWidth = itemWidth = f.itemWidth || symbolWidth
						+ symbolPadding + d.width + rightPadding;
				if (g) {
					itemX += itemWidth;
					offsetWidth = widthOption
							|| mathMax(itemX - initialItemX, offsetWidth);
					if (itemX - initialItemX + itemWidth > (widthOption || (chartWidth
			...
			function renderLegend() {
				...
				each(series, function(a) {
							...
							var b = (a.options.legendType == 'point')
									? a.data
									: [a];
							each(b, renderItem)
						});
				...
				if (legendBorderWidth || legendBackgroundColor) {
					...
					boxHeight += 2 * padding;
					if (!box) {
						box = renderer.rect(0, 0, boxWidth, boxHeight,
				...
				while (i--) {
					...
					if (style[prop] && style[prop] != 'auto') {
						...
						f[i < 2 ? 'x' : 'y'] = parseInt(style[prop], 10)
								* (i % 2 ? -1 : 1)
					}
				}
		...
		isInsidePlot = function(x, y) {
			var a = 0, top = 0;
			return x >= a && x <= a + plotWidth && y >= top
					&& y <= top + plotHeight
		};
		...
		function showLoading(a) {
			...
			if (!loadingShown) {
				...
				loadingLayer.getElementsByTagName('span')[0].innerHTML = a
						|| n.lang.loading;
		...
		function showTitle() {
			...
			if (a && a.text) {
				renderer.text(a.text, anchorMap[titleAlign] + a.x, a.y,
						a.style, 0, titleAlign).attr({
							'class' : 'highcharts-title'
						}).add()
			}
			if (subtitle && subtitle.text) {
				renderer.text(subtitle.text,
						anchorMap[subtitleAlign] + subtitle.x, subtitle.y,
						subtitle.style, 0, subtitleAlign).attr({
							'class' : 'highcharts-subtitle'
						}).add()
			}
		}
		getAlignment = function(a) {
			var b = a.align, vAlign = a.verticalAlign, optionsX = a.x || 0, optionsY = a.y
					|| 0, ret = {
				x : optionsX || 0,
				y : optionsY || 0
			};
		...
		function getContainer() {
			...
			chart.chartWidth = chartWidth = p.width
					|| (renderToClone || renderTo).offsetWidth || 600;
			chart.chartHeight = chartHeight = p.height
					|| (a > plotTop + marginBottom ? a : 0) || 400;
			chart.plotWidth = plotWidth = chartWidth - plotLeft - marginRight;
			chart.plotHeight = plotHeight = chartHeight - plotTop
					- marginBottom;
			...
			chart.renderer = renderer = p.renderer == 'SVG' ? new D(container,
					chartWidth, chartHeight) : new G(container, chartWidth,
					chartHeight)
		}
		function render() {
			var b, labels = n.labels, credits = n.credits, chartBorderWidth = p.borderWidth
					|| 0, chartBackgroundColor = p.backgroundColor, plotBackgroundColor = p.plotBackgroundColor, plotBackgroundImage = p.plotBackgroundImage;
			b = 2 * chartBorderWidth + (p.shadow ? 8 : 0);
			if (chartBorderWidth || chartBackgroundColor) {
				renderer.rect(b / 2, b / 2, chartWidth - b, chartHeight - b,
						p.borderRadius, chartBorderWidth).attr({
			...
			if (plotBackgroundImage) {
				renderer.image(plotBackgroundImage, plotLeft, plotTop,
						plotWidth, plotHeight).add()
			}
			if (p.plotBorderWidth) {
				renderer.rect(plotLeft, plotTop, plotWidth, plotHeight, 0,
						p.plotBorderWidth).attr({
			...
			if (labels.items) {
				each(labels.items, function() {
							var a = extend(labels.style, this.style), x = parseInt(
									a.left, 10)
									+ plotLeft, y = parseInt(a.top, 10)
									+ plotTop + 12;
							delete a.left;
							delete a.top;
			...
			if (credits.enabled && !chart.credits) {
				renderer.text(credits.text, chartWidth - 10, chartHeight - 5,
						credits.style, 0, 'right').on('click', function() {
	...
	H.prototype = {
		init : function(a, b) {
			...
			if (a.options.colorByPoint) {
				...
				c.color = c.options.color = c.color
						|| defaultColors[colorCounter++];
		...
		firePointEvent : function(b, c, d) {
			...
			if (seriesOptions.point.events[b]
		...
		setState : function(a) {
			var b = this, series = b.series, stateOptions = series.options.states, markerOptions = series.options.marker, normalDisabled = markerOptions
					&& !markerOptions.enabled, markerStateOptions = markerOptions
					&& markerOptions.states[a], stateDisabled = markerStateOptions
					&& markerStateOptions.enabled === false, chart = series.chart, pointAttr = b.pointAttr;
			if (!a) {
				a = NORMAL_STATE
			}
			if ((b.selected && a != SELECT_STATE)
					|| (stateOptions[a] && stateOptions[a].enabled === false)
					|| (a && (stateDisabled || normalDisabled
							&& !markerStateOptions.enabled))) {
				return
			}
			if (a && !b.graphic) {
				if (!series.stateMarkerGraphic) {
					series.stateMarkerGraphic = chart.renderer.circle(0, 0,
							pointAttr[a].r).attr(pointAttr[a])
							.add(series.group)
				}
	...
	I.prototype = {
		...
		setOptions : function(a) {
			var b = this.chart.options.plotOptions, options = merge(
					b[this.type], b.series, a);
			return options
		},
		getColor : function() {
		...
		getSymbol : function() {
			var a = this.chart.options.symbols, symbol = this.options.marker.symbol
					|| a[symbolCounter++];
			this.symbol = symbol;
		...
		addPoint : function(a, b, c) {
			var d = this, data = d.data, point = (new d.pointClass())
					.init(d, a);
			b = pick(b, true);
			data.push(point);
			if (c) {
		...
		setData : function(b, c) {
			var d = this, oldData = d.data, initialColor = d.initialColor, i = oldData
					&& oldData.length || 0;
			d.xIncrement = null;
		...
		translate : function() {
			...
			while (i--) {
				...
				if (stacking && a.visible && stack[xValue]) {
					...
					b.percentage = pointStackTotal ? b.y * 100
							/ pointStackTotal : 0;
				...
				b.clientX = chart.inverted
						? chart.plotHeight - b.plotX
						: b.plotX;
				b.category = categories && categories[b.x] !== UNDEFINED
						? categories[b.x]
						: b.x
			}
		},
		setTooltipPoints : function(b) {
			var c = this, chart = c.chart, inverted = chart.inverted, data = [], plotSize = (inverted
					? chart.plotTop
					: chart.plotLeft)
					+ chart.plotSizeX, low, high, tooltipPoints = [];
			if (b) {
			...
			each(data, function(a, i) {
						...
						high = a.high = data[i + 1]
								? (mathFloor((a.plotX + (data[i + 1] ? data[i
										+ 1].plotX : plotSize))
										/ 2))
								: plotSize;
						while (low <= high) {
						...
						if (graphic) {
							...
							point.graphic = chart.renderer.symbol(
									pick(point.marker && point.marker.symbol,
											a.symbol), plotX, plotY, radius)
									.attr(pointAttr).add(a.group)
						}
					}
				}
			}
		...
		getAttribs : function() {
			...
			if (b.options.marker) {
				...
				stateOptionsHover.color = stateOptionsHover.color
						|| C(stateOptionsHover.color || seriesColor)
								.brighten(stateOptionsHover.brightness).get()
			}
			seriesPointAttr[NORMAL_STATE] = b.convertAttribs(normalOptions,
					normalDefaults);
			each([HOVER_STATE, SELECT_STATE], function(a) {
						seriesPointAttr[a] = b.convertAttribs(stateOptions[a],
			...
			while (i--) {
				...
				if (hasPointSpecificOptions) {
					...
					pointStateOptionsHover = stateOptions[HOVER_STATE] = stateOptions[HOVER_STATE]
							|| {};
					if (!b.options.marker) {
						pointStateOptionsHover.color = C(pointStateOptionsHover.color
								|| point.options.color)
								.brighten(pointStateOptionsHover.brightness
										|| stateOptionsHover.brightness).get()
					}
					pointAttr[NORMAL_STATE] = b.convertAttribs(normalOptions,
							seriesPointAttr[NORMAL_STATE]);
					pointAttr[HOVER_STATE] = b.convertAttribs(
							stateOptions[HOVER_STATE],
							seriesPointAttr[HOVER_STATE],
							pointAttr[NORMAL_STATE]);
					pointAttr[SELECT_STATE] = b.convertAttribs(
		...
		drawDataLabels : function() {
			if (this.options.dataLabels.enabled) {
				...
				if (!dataLabelsGroup) {
					dataLabelsGroup = c.dataLabelsGroup = chart.renderer
							.g(PREFIX + 'data-labels').attr({
				...
				each(data, function(a) {
					...
					if (str) {
						a.dataLabel = chart.renderer.text(str, x, y,
								options.style, options.rotation, align).attr({
							zIndex : 1,
							visibility : a.visible === false
									? HIDDEN
									: 'inherit'
						}).add(dataLabelsGroup)
					}
		...
		drawGraph : function(e) {
			var f = this, options = f.options, chart = f.chart, graph = f.graph, graphPath = [], fillColor, area = f.area, group = f.group, color = options.lineColor
					|| f.color, lineWidth = options.lineWidth, segmentPath, renderer = chart.renderer, translatedThreshold = f.yAxis
					.getThreshold(options.threshold || 0,
							!f.chart.options.FCconf.negative
									&& f.yAxis.options.PCreversed), useArea = /^area/
					.test(f.type), singlePoints = [], areaPath = [];
						...
						if (c.length > 1) {
							...
							if (useArea) {
								...
								if (options.stacking && f.type != 'areaspline') {
									...
									d.push(c[c.length - 1].plotX,
											translatedThreshold, c[0].plotX,
											translatedThreshold, 'z')
								}
			...
			if (useArea) {
				fillColor = pick(options.fillColor, C(f.color)
		...
		render : function() {
			var a = this, chart = a.chart, group, doAnimation = a.options.animation
					&& a.animate, renderer = chart.renderer;
			if (!a.clipRect) {
				a.clipRect = !chart.hasRendered && chart.clipRect
						? chart.clipRect
						: renderer.clipRect(0, 0, chart.plotSizeX,
								chart.plotSizeY);
				if (!chart.clipRect) {
	...
	function SplineHelper(a) {
		...
		for (i = 1; i < n - 1; i++) {
			...
			this.y2[i] = (s - 1.0) / p;
			e[i] = (c[i + 1] - c[i]) / (b[i + 1] - b[i]) - (c[i] - c[i - 1])
					/ (b[i] - b[i - 1]);
		...
		interpolate : function(c) {
			...
			return a
					* this.ydata[intMin]
					+ b
					* this.ydata[f]
					+ ((a * a * a - a) * this.y2[intMin] + (b * b * b - b)
							* this.y2[f]) * (h * h) / 6.0
		}
	};
	var N = extendClass(I, {
		...
		getSplineData : function() {
			...
			each(d.segments, function(b) {
						...
						each(b, function(a, i) {
									...
									if (nextUp.plotX >= 0
											&& nextDown.plotX <= plotSizeX) {
										c.push(a)
									}
								});
						if (c.length > 1) {
							num = mathRound(mathMax(plotSizeX,
									c[c.length - 1].clientX - c[0].clientX)
									/ 3)
						}
						splinedata.push(b.length > 1 ? num
								? (new SplineHelper(c)).get(num)
								: [] : b)
					});
			return splinedata
		}
	...
	var P = extendClass(I, {
		...
		translate : function() {
			...
			var d = c.options, data = c.data, closestPoints = c.closestPoints, categoryWidth = mathAbs(data[1]
					? data[closestPoints].plotX - data[closestPoints - 1].plotX
					: chart.plotSizeX / (categories ? categories.length : 1)), groupPadding = categoryWidth
					* d.groupPadding, groupWidth = categoryWidth - 2
					* groupPadding, pointOffsetWidth = groupWidth / columnCount, optionPointWidth = d.pointWidth, pointPadding = defined(optionPointWidth)
					? (pointOffsetWidth - optionPointWidth) / 2
					: pointOffsetWidth * d.pointPadding, pointWidth = pick(
					optionPointWidth, pointOffsetWidth - 2 * pointPadding), columnIndex = (reversedXAxis
					? columnCount - c.columnIndex
					: c.columnIndex)
					|| 0, pointXOffset = pointPadding
					+ (groupPadding + columnIndex * pointOffsetWidth - (categoryWidth / 2))
					* (reversedXAxis ? -1 : 1), translatedThreshold = c.yAxis
					.getThreshold(d.threshold || 0), minPointLength = d.minPointLength;
			each(data, function(a) {
				var b = pick(a.MX, a.plotX + pointXOffset), barH = mathCeil(mathAbs((a.yBottom || translatedThreshold)
						- a.plotY)), plotY = a.MY ? translatedThreshold
						- ((barH / a.y) * a.MY) : a.plotY, barY = mathCeil(mathMin(
						plotY, translatedThreshold)), barW = pick(a.MWidth,
						pointWidth), trackerY;
				if (mathAbs(barH) < (minPointLength || 5)) {
					if (minPointLength) {
						barH = minPointLength;
						barY = translatedThreshold
				...
				if (!c.chart.options.FCconf.negative
						&& a.series.yAxis.options.PCreversed === true) {
					barY = barH;
			...
			each(b.data, function(a) {
						if (defined(a.plotY)) {
							...
							if (graphic) {
								...
								a.graphic = renderer[a.shapeType](shapeArgs)
										.attr(a.pointAttr[a.selected
												? SELECT_STATE
												: NORMAL_STATE]).add(b.group)
	...
	var T = extendClass(I, {
		...
		translate : function() {
			var b = 0, series = this, cumulative = -0.25, options = series.options, slicedOffset = options.slicedOffset, positions = options.center, chart = series.chart, plotWidth = chart.plotWidth, plotHeight = chart.plotHeight, start, end, angle, data = series.data, circ = 2
					* math.PI, fraction, smallestSize = mathMin(plotWidth,
					plotHeight), isPercent;
			positions.push(options.size, options.innerSize || 0);
			positions = map(positions, function(a, i) {
						isPercent = /%$/.test(a);
						return isPercent ? [plotWidth, plotHeight,
								smallestSize, smallestSize][i]
								* parseInt(a, 10) / 100 : a
					});
			each(data, function(a) {
						b += a.y
			...
			each(data, function(a) {
						...
						angle = (end + start) / 2;
						a.slicedTranslation = map([
										mathCos(angle) * slicedOffset
												+ chart.plotLeft,
										mathSin(angle) * slicedOffset
												+ chart.plotTop], mathRound);
						a.tooltipPos = [
								positions[0] + mathCos(angle) * positions[2]
										* 0.35,
								positions[1] + mathSin(angle) * positions[2]
										* 0.35];
		...
		drawPoints : function() {
			...
			each(b.data, function(a) {
						...
						shapeArgs = a.shapeArgs;
						if (!a.group) {
							groupTranslation = a.sliced
									? a.slicedTranslation
									: [chart.plotLeft, chart.plotTop];
							a.group = renderer.g('point').attr({
										zIndex : 3
									}).add().translate(groupTranslation[0],
									groupTranslation[1])
						}
						if (graphic) {
							graphic.attr(shapeArgs)
						} else {
							a.graphic = renderer.arc(shapeArgs)
									.attr(a.pointAttr[NORMAL_STATE])
									.add(a.group)
						}
						if (a.visible === false) {
							a.setVisible(false)
...
(function() {
	...
	extend(Chart.prototype, {
		getSVG : function(d) {
			var e = this, chartCopy, sandbox, svg, seriesOptions, pointOptions, pointMarker, options = merge(
					e.options, d);
			...
			each(e.series, function(b) {
						...
						seriesOptions.data = [];
						each(b.data, function(a) {
									pointOptions = a.config == null
											|| typeof a.config == 'number' ? {
										y : a.y
									} : a.config;
									pointOptions.x = a.x;
			...
			svg = svg.replace(/zIndex="[^"]+"/g, '').replace(
					...
					.replace(/jQuery[0-9]+="[^"]+"/g, '').replace(
							...
							'url(#').replace(/id=([^" >]+)/g, 'id="$1"')
					.replace(/class=([^" ]+)/g, 'class="$1"').replace(
							/ transform /g, ' ').replace(/:path/g, 'path')
		...
		contextMenu : function(b, c, x, y, d, e) {
			var f = this, navOptions = f.options.navigation, menuItemStyle = navOptions.menuItemStyle, chartWidth = f.chartWidth, chartHeight = f.chartHeight, cacheName = 'cache-'
					+ b, menu = f[cacheName], menuPadding = mathMax(d, e), boxShadow = '3px 3px 10px #888', innerMenu, hide, menuStyle;
			if (!menu) {
		...
		addButton : function(a) {
			var b = this, renderer = b.renderer, btnOptions = merge(
					b.options.navigation.buttonOptions, a), onclick = btnOptions.onclick, menuItems = btnOptions.menuItems, position = b
					.getAlignment(btnOptions), buttonLeft = position.x, buttonTop = position.y, buttonWidth = btnOptions.width, buttonHeight = btnOptions.height, box, symbol, button, borderWidth = btnOptions.borderWidth, boxAttr = {
			...
			box = renderer.rect(0, 0, buttonWidth, buttonHeight,
					btnOptions.borderRadius, borderWidth).translate(buttonLeft,
					buttonTop).attr(extend({
						fill : btnOptions.backgroundColor,
						'stroke-width' : borderWidth,
						zIndex : 19
					}, boxAttr)).add();
			button = renderer.rect(buttonLeft, buttonTop, buttonWidth,
					buttonHeight, 0).attr({
			...
			if (menuItems) {
				onclick = function(e) {
					b.contextMenu('export-menu', menuItems, buttonLeft,
			...
			symbol = renderer.symbol(btnOptions.symbol,
					buttonLeft + btnOptions.symbolX,
					buttonTop + btnOptions.symbolY,
					(btnOptions.symbolSize || 12) / 2).attr(extend(symbolAttr,
					{
	...
	g.Renderer.prototype.symbols.exportIcon = function(x, y, a) {
		return [M, x - a, y + a, L, x + a, y + a, x + a, y + a * 0.5, x - a,
				y + a * 0.5, 'Z', M, x, y + a * 0.5, L, x - a * 0.5, y - a / 3,
				x - a / 6, y - a / 3, x - a / 6, y - a, x + a / 6, y - a,
				x + a / 6, y - a / 3, x + a * 0.5, y - a / 3, 'Z']
	};
	g.Renderer.prototype.symbols.printIcon = function(x, y, a) {
		return [M, x - a, y + a * 0.5, L, x + a, y + a * 0.5, x + a, y - a / 3,
				x - a, y - a / 3, 'Z', M, x - a * 0.5, y - a / 3, L,
				x - a * 0.5, y - a, x + a * 0.5, y - a, x + a * 0.5, y - a / 3,
				'Z', M, x - a * 0.5, y + a * 0.5, L, x - a * 0.75, y + a,
				x + a * 0.75, y + a, x + a * 0.5, y + a * 0.5, 'Z']
	};
